import Nucleus.HolJson.Syntax
import Nucleus.Json.Raw

/-!
# The v0 HOL JSON tree codec

This module formalizes the initial Rust/Serde representation.  Every node is
an object whose first member is `"tag"`; remaining members are the fields of
the fixed struct wrapped by that enum variant.  Children are nested JSON
trees.  The decoder is intentionally strict about member order, spelling,
arity, and scalar kinds, matching the canonical output produced by Serde.

Strict raw decoding gives a small, auditable canonical-output language.  It is
not the semantic JSON boundary: ordinary JSON objects are unordered, so the
extensional codec layered above this module accepts member permutations after
duplicate-key validation.
-/

namespace Nucleus.HolJson

/-- Scalar kinds used by the v0 HOL JSON profile. -/
inductive Scalar where
  | string (value : String)
  | uint (value : UInt64)
  | bool (value : Bool)
  deriving DecidableEq, Repr

abbrev Json := RawJson Scalar

namespace Codec

private def string (value : String) : Json := .scalar (.string value)
private def uint (value : UInt64) : Json := .scalar (.uint value)
private def bool (value : Bool) : Json := .scalar (.bool value)

private def field (key : String) (value : Json) (tail : RawSyn Scalar .obj) :
    RawSyn Scalar .obj := .objCons key value tail

private def tagged (tag : String) (fields : RawSyn Scalar .obj := .objNil) : Json :=
  .map (.objCons "tag" (string tag) fields)

/-- Serialize a raw HOL tree to the exact v0 nested JSON syntax. -/
def encode : WireSyntax -> Json
  | .base name => tagged "ty.base" (field "name" (string name) .objNil)
  | .boolTy => tagged "ty.bool"
  | .indTy => tagged "ty.ind"
  | .arr domain codomain =>
      tagged "ty.arr" (field "domain" (encode domain)
        (field "codomain" (encode codomain) .objNil))
  | .sub carrier predicate =>
      tagged "ty.sub" (field "carrier" (encode carrier)
        (field "predicate" (encode predicate) .objNil))
  | .bound index => tagged "tm.bound" (field "index" (uint index) .objNil)
  | .free name => tagged "tm.free" (field "name" (uint name) .objNil)
  | .app function argument =>
      tagged "tm.app" (field "function" (encode function)
        (field "argument" (encode argument) .objNil))
  | .lam domain body =>
      tagged "tm.lam" (field "domain" (encode domain)
        (field "body" (encode body) .objNil))
  | .bool value => tagged "tm.bool" (field "value" (bool value) .objNil)
  | .zero => tagged "tm.zero"
  | .succ value => tagged "tm.succ" (field "value" (encode value) .objNil)
  | .eqn type left right =>
      tagged "tm.eq" (field "type" (encode type)
        (field "left" (encode left) (field "right" (encode right) .objNil)))
  | .eps type predicate =>
      tagged "tm.eps" (field "type" (encode type)
        (field "predicate" (encode predicate) .objNil))
  | .abs carrier predicate value =>
      tagged "tm.abs" (field "carrier" (encode carrier)
        (field "predicate" (encode predicate) (field "value" (encode value) .objNil)))
  | .rep carrier predicate value =>
      tagged "tm.rep" (field "carrier" (encode carrier)
        (field "predicate" (encode predicate) (field "value" (encode value) .objNil)))

private def decodeFuel : Nat -> Json -> Option WireSyntax
  | 0, _ => none
  | fuel + 1, .map (.objCons key (.scalar (.string tag)) fields) =>
      if key != "tag" then none
      else
      if tag = "ty.base" then
        match fields with
        | .objCons key (.scalar (.string name)) .objNil =>
            if key = "name" then some (.base name) else none
        | _ => none
      else if tag = "ty.bool" then
        match fields with | .objNil => some .boolTy | _ => none
      else if tag = "ty.ind" then
        match fields with | .objNil => some .indTy | _ => none
      else if tag = "ty.arr" then
        match fields with
        | .objCons first domain (.objCons second codomain .objNil) =>
            if first = "domain" ∧ second = "codomain" then
              return .arr (← decodeFuel fuel domain) (← decodeFuel fuel codomain)
            else none
        | _ => none
      else if tag = "ty.sub" then
        match fields with
        | .objCons first carrier (.objCons second predicate .objNil) =>
            if first = "carrier" ∧ second = "predicate" then
              return .sub (← decodeFuel fuel carrier) (← decodeFuel fuel predicate)
            else none
        | _ => none
      else if tag = "tm.bound" then
        match fields with
        | .objCons key (.scalar (.uint index)) .objNil =>
            if key = "index" then some (.bound index) else none
        | _ => none
      else if tag = "tm.free" then
        match fields with
        | .objCons key (.scalar (.uint name)) .objNil =>
            if key = "name" then some (.free name) else none
        | _ => none
      else if tag = "tm.app" then
        match fields with
        | .objCons first function (.objCons second argument .objNil) =>
            if first = "function" ∧ second = "argument" then
              return .app (← decodeFuel fuel function) (← decodeFuel fuel argument)
            else none
        | _ => none
      else if tag = "tm.lam" then
        match fields with
        | .objCons first domain (.objCons second body .objNil) =>
            if first = "domain" ∧ second = "body" then
              return .lam (← decodeFuel fuel domain) (← decodeFuel fuel body)
            else none
        | _ => none
      else if tag = "tm.bool" then
        match fields with
        | .objCons key (.scalar (.bool value)) .objNil =>
            if key = "value" then some (.bool value) else none
        | _ => none
      else if tag = "tm.zero" then
        match fields with | .objNil => some .zero | _ => none
      else if tag = "tm.succ" then
        match fields with
        | .objCons key value .objNil =>
            if key = "value" then return .succ (← decodeFuel fuel value) else none
        | _ => none
      else if tag = "tm.eq" then
        match fields with
        | .objCons first type (.objCons second left (.objCons third right .objNil)) =>
            if first = "type" ∧ second = "left" ∧ third = "right" then
              return .eqn (← decodeFuel fuel type) (← decodeFuel fuel left)
                (← decodeFuel fuel right)
            else none
        | _ => none
      else if tag = "tm.eps" then
        match fields with
        | .objCons first type (.objCons second predicate .objNil) =>
            if first = "type" ∧ second = "predicate" then
              return .eps (← decodeFuel fuel type) (← decodeFuel fuel predicate)
            else none
        | _ => none
      else if tag = "tm.abs" then
        match fields with
        | .objCons first carrier
            (.objCons second predicate (.objCons third value .objNil)) =>
            if first = "carrier" ∧ second = "predicate" ∧ third = "value" then
              return .abs (← decodeFuel fuel carrier) (← decodeFuel fuel predicate)
                (← decodeFuel fuel value)
            else none
        | _ => none
      else if tag = "tm.rep" then
        match fields with
        | .objCons first carrier
            (.objCons second predicate (.objCons third value .objNil)) =>
            if first = "carrier" ∧ second = "predicate" ∧ third = "value" then
              return .rep (← decodeFuel fuel carrier) (← decodeFuel fuel predicate)
                (← decodeFuel fuel value)
            else none
        | _ => none
      else none
  | _ + 1, _ => none

private def height : WireSyntax -> Nat
  | .base _ | .boolTy | .indTy | .bound _ | .free _ | .bool _ | .zero => 0
  | .arr a b | .sub a b | .app a b | .lam a b | .eps a b =>
      1 + max (height a) (height b)
  | .succ value => 1 + height value
  | .eqn a b c | .abs a b c | .rep a b c =>
      1 + max (height a) (max (height b) (height c))

private theorem succ_lt_succ_iff' (a n : Nat) : 1 + a < n + 1 ↔ a < n := by
  omega

/-- Parse one exact v0 nested HOL JSON tree.  Fuel is derived from the input's
JSON depth, so decoding is total even for adversarial trees. -/
def decode (json : Json) : Option WireSyntax := decodeFuel (json.depth + 1) json

private theorem decodeFuel_encode (term : WireSyntax) (fuel : Nat)
    (enough : height term < fuel) : decodeFuel fuel (encode term) = some term := by
  induction term generalizing fuel <;> cases fuel <;>
    simp_all [height, encode, tagged, field, string, uint, bool, decodeFuel,
      succ_lt_succ_iff', Nat.max_lt]

private theorem height_lt_encode_depth (term : WireSyntax) :
    height term < (encode term).depth + 1 := by
  induction term <;>
    simp_all [height, encode, tagged, field, string, uint, bool, RawSyn.depth] <;> omega

/-- Serialization followed by deserialization is the identity on every raw
HOL tree. -/
@[simp] theorem decode_encode (term : WireSyntax) : decode (encode term) = some term := by
  exact decodeFuel_encode term ((encode term).depth + 1) (height_lt_encode_depth term)

/-- The v0 serializer is injective, as an immediate consequence of the
verified decoder. -/
theorem encode_injective : Function.Injective encode :=
  fun {left right} equality => Option.some.inj (by
    rw [← decode_encode left, ← decode_encode right, equality])

end Codec

end Nucleus.HolJson
