...
def children : detail.Expr → List Ref
  ...
  | .op1 _ child => [child]
  | .tyFv _ child | .tyExists _ child | .tyForall _ child | .model _ child | .tmFv _ child => [child]
...
def extras (row : detail.Row) : List RowExtra :=
  let expression := match row.expr with
    | .tyFv name _ | .tyExists name _ | .tyForall name _ | .model name _ | .tmFv name _ => [.nat name]
