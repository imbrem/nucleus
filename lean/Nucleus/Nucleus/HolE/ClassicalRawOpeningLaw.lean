...
/-- The raw-typed opening fact needed by predicate-sensitive subtype rules.

The context validity arguments make explicit the condition already imposed by
`CEntails`: every polymorphic bound entry is one value viewed at different
expected types, rather than unrelated values at each expectation. -/
