...
theorem TypedHyps.substTy {Base : Type u} {Target : Type v}
    ...
    {H : List (Tm Base depth)} (typed : TypedHyps Δ Γ H) :
    TypedHyps (substFreeCtx σ Δ) (substBoundCtx σ Γ) (substHyps σ H) := by
  intro p member
...
def EqTm.substTy {Base : Type u} {Target : Type v} {σ : TypeSub Base Target}
    ...
    {Δ : FreeCtx Base} -> {depth : Nat} -> {Γ : BoundCtx Base depth} ->
    {t uterm : Tm Base depth} -> {A : Ty Base} -> EqTm Δ Γ t uterm A ->
      EqTm (substFreeCtx σ Δ) (substBoundCtx σ Γ)
        (substTm σ t) (substTm σ uterm) (Nucleus.HolLN.substTy σ A)
  | _, _, _, _, _, _, .refl typing => .refl (typing.substTy admissible.wellKinded)
  | _, _, _, _, _, _, .symm equality => .symm (EqTm.substTy admissible equality)
  | _, _, _, _, _, _, .trans first second =>
      .trans (EqTm.substTy admissible first) (EqTm.substTy admissible second)
  | _, _, _, _, _, _, .app function argument =>
      .app (EqTm.substTy admissible function) (EqTm.substTy admissible argument)
  | _, _, _, _, _, _, .succ equality => .succ (EqTm.substTy admissible equality)
  | _, _, Γ, _, _, _, .lam hA equality => by
      apply EqTm.lam (hA.substTy admissible.wellKinded)
      simpa [substBoundCtx_extend] using EqTm.substTy admissible equality
  | _, _, Γ, _, _, _, .beta body x hA bodyTyping argumentTyping resultTyping => by
  ...
  | _, _, _, _, _, _, .eta name fresh functionTyping etaTyping => by
...
def Proves.substTy {Base : Type u} {Target : Type v} {σ : TypeSub Base Target}
    (admissible : AdmissibleTypeSub σ) :
    {Δ : FreeCtx Base} -> {depth : Nat} -> {Γ : BoundCtx Base depth} ->
    {H : List (Tm Base depth)} -> {p : Tm Base depth} -> Proves Δ Γ H p ->
      Proves (substFreeCtx σ Δ) (substBoundCtx σ Γ)
        (substHyps σ H) (substTm σ p)
  | _, _, _, _, _, .hyp typed member =>
      .hyp (typed.substTy admissible.wellKinded) (List.mem_map_of_mem member)
  | _, _, _, _, _, .truth typed => .truth (typed.substTy admissible.wellKinded)
  | _, _, _, _, _, .eqRefl typed hA hx =>
      .eqRefl (typed.substTy admissible.wellKinded)
        (hA.substTy admissible.wellKinded) (hx.substTy admissible.wellKinded)
  | _, _, _, _, _, .eqMp typed hA hp hx hy equality proof =>
  ...
  | _, _, _, _, _, .absRep typed hA hp hx => by
      apply Proves.absRep (typed.substTy admissible.wellKinded)
        (hA.substTy admissible.wellKinded)
      · simpa [Nucleus.HolLN.substTy, substTm, substHol, substFreeCtx_empty, substBoundCtx_empty,
          substBoundCtx_extend] using
          hp.substTy admissible.wellKinded
      · exact hx.substTy admissible.wellKinded
  | _, _, _, _, _, .repAbs typed hA hp hx predicateTyping premise => by
  ...
  | _, _, _, _, _, .succInjective typed hx hy premise =>
      .succInjective (typed.substTy admissible.wellKinded)
        (hx.substTy admissible.wellKinded) (hy.substTy admissible.wellKinded)
        (Proves.substTy admissible premise)
  | _, _, _, _, _, .zeroNotSucc typed hx =>
