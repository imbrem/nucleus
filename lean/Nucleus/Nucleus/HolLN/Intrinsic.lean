...
depth-zero specialization: its terms have no unbound de Bruijn variables;
free variables carry their own syntactic types.
...
abbrev BoolTm {Base : Type u} {depth : Nat}
    (Γ : BoundCtx Base depth) := Checked Γ (.boolTy : Ty Base)

/-- Proposition-oriented spelling for APIs that treat Boolean terms logically. -/
abbrev HolProp {Base : Type u} {depth : Nat}
    (Γ : BoundCtx Base depth) := BoolTm Γ

/-- A term intrinsically certified to have the distinguished natural type. -/
abbrev NatTm {Base : Type u} {depth : Nat}
    (Γ : BoundCtx Base depth) := Checked Γ (.natTy : Ty Base)

/-- A proposition context whose typing invariant is carried by its entries. -/
abbrev PropCtx {Base : Type u} {depth : Nat}
    (Γ : BoundCtx Base depth) := List (BoolTm Γ)

/-- A proposition context with no unbound de Bruijn term variables. -/
abbrev ClosedPropCtx {Base : Type u} :=
  PropCtx (emptyBound : BoundCtx Base 0)
...
noncomputable def ofProp {Base : Type u} {depth : Nat}
    {Γ : BoundCtx Base depth} (proposition : Prop) : BoolTm Γ := by
  classical
  exact if proposition then boolean true else boolean false

noncomputable instance {Base : Type u} {depth : Nat}
    {Γ : BoundCtx Base depth} : Coe Prop (BoolTm Γ) where
  coe := ofProp

@[simp] theorem ofProp_eq_true {Base : Type u} {depth : Nat}
    {Γ : BoundCtx Base depth} (proposition : Prop) (holds : proposition) :
    (ofProp proposition : BoolTm Γ) = boolean true := by
  simp [ofProp, holds]

@[simp] theorem ofProp_eq_false {Base : Type u} {depth : Nat}
    {Γ : BoundCtx Base depth} (proposition : Prop) (fails : ¬ proposition) :
    (ofProp proposition : BoolTm Γ) = boolean false := by
  simp [ofProp, fails]

def app {Base : Type u} {depth : Nat}
...
instance {Base : Type u} {depth : Nat}
    {Γ : BoundCtx Base depth} : Coe Nat (NatTm Γ) where
  coe := natural

instance {Base : Type u} {depth : Nat}
    {Γ : BoundCtx Base depth} (n : Nat) : OfNat (NatTm Γ) n where
  ofNat := natural n
...
def boolImp {Base : Type u} {depth : Nat}
    {Γ : BoundCtx Base depth} (left right : BoolTm Γ) : BoolTm Γ :=
  boolOr (boolNot left) right

instance {Base : Type u} {depth : Nat}
    {Γ : BoundCtx Base depth} : Top (BoolTm Γ) := ⟨boolean true⟩

instance {Base : Type u} {depth : Nat}
    {Γ : BoundCtx Base depth} : Bot (BoolTm Γ) := ⟨boolean false⟩

instance {Base : Type u} {depth : Nat}
    {Γ : BoundCtx Base depth} : Min (BoolTm Γ) := ⟨boolAnd⟩

instance {Base : Type u} {depth : Nat}
    {Γ : BoundCtx Base depth} : Max (BoolTm Γ) := ⟨boolOr⟩

instance {Base : Type u} {depth : Nat}
    {Γ : BoundCtx Base depth} : HAnd (BoolTm Γ) (BoolTm Γ) (BoolTm Γ) :=
  ⟨boolAnd⟩

instance {Base : Type u} {depth : Nat}
    {Γ : BoundCtx Base depth} : HOr (BoolTm Γ) (BoolTm Γ) (BoolTm Γ) :=
  ⟨boolOr⟩

instance {Base : Type u} {depth : Nat}
    {Γ : BoundCtx Base depth} : Compl (BoolTm Γ) := ⟨boolNot⟩

instance {Base : Type u} {depth : Nat}
    {Γ : BoundCtx Base depth} : HNot (BoolTm Γ) := ⟨boolNot⟩

instance {Base : Type u} {depth : Nat}
    {Γ : BoundCtx Base depth} : HImp (BoolTm Γ) := ⟨boolImp⟩
...
@[simp] theorem terms_ofTyped {Base : Type u} {depth : Nat}
    {Γ : BoundCtx Base depth} (hypotheses : List (Tm Base depth))
    (typed : TypedHyps Γ hypotheses) :
...
class HasConjoin {Base : Type u} {depth : Nat}
    (Γ : BoundCtx Base depth) where
  conjoin : PropCtx Γ -> BoolTm Γ
...
def eqRefl (hA : Kinded A) (x : Checked Γ A) :
    Proves Γ H (Checked.eq hA x x) := ⟨.eqRefl (PropCtx.typed H) hA x.typing⟩

def eqMp (hA : Kinded A) (predicate : Checked Γ (.arr A .boolTy))
    (x y : Checked Γ A) (equality : Proves Γ H (Checked.eq hA x y))
...
def choice (hA : Kinded A) (predicate : Checked Γ (.arr A .boolTy))
    (x : Checked Γ A) (premise : Proves Γ H (Checked.app predicate x)) :
...
def eqOfEqTm (hA : Kinded A) {x y : Checked Γ A} (equality : EqTm Γ x y) :
...
theorem nonemptyProvesOfProp {Base : Type u} (proposition : Prop)
    (holds : proposition) :
    Nonempty (Proves (emptyBound : BoundCtx Base 0) []
...
noncomputable def provesOfProp {Base : Type u} (proposition : Prop)
    (holds : proposition) :
    Proves (emptyBound : BoundCtx Base 0) []
...
theorem propOfProves {Base : Type u} (proposition : Prop)
    (proof : Proves (emptyBound : BoundCtx Base 0) []
      (Checked.ofProp proposition)) : proposition := by
  classical
  by_cases holds : proposition
  · exact holds
  · have equality := Checked.ofProp_eq_false (Base := Base)
      (Γ := emptyBound) proposition holds
    have falseProof : Proves (emptyBound : BoundCtx Base 0) [] (Checked.boolean false) :=
      equality ▸ proof
