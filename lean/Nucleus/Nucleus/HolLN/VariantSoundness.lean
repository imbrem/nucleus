...
def Proves (Γ : BoundCtx Base depth)
    (H : List (Tree.Sorted Base .tm)) (p : Tree.Sorted Base .tm) : Prop :=
  ∃ H₀ p₀, Nonempty (Nucleus.HolLN.Proves Γ H₀ p₀) ∧
    H₀.map Erasure.toSorted = H ∧ Erasure.toSorted p₀ = p

theorem HasType.sound (h : HasType Γ t A) :
    ∃ t₀ A₀, Nucleus.HolLN.HasType Γ t₀ A₀ ∧ Erasure.toSorted t₀ = t ∧
      Erasure.toSorted A₀ = A ∧ ∀ (freeEnv : FreeEnv Base) (boundEnv : BoundEnv Γ),
        ∃ value : DenoteTy A₀, Eval Γ freeEnv boundEnv t₀ A₀ value := by
  rcases h with ⟨t₀, A₀, typing, rfl, rfl⟩
  exact ⟨t₀, A₀, typing, rfl, rfl, typing.eval_exists⟩

theorem EqTm.sound (h : EqTm Γ t u A) :
    ∃ t₀ u₀ A₀, Nonempty (Nucleus.HolLN.EqTm Γ t₀ u₀ A₀) ∧
      Erasure.toSorted t₀ = t ∧ Erasure.toSorted u₀ = u ∧ Erasure.toSorted A₀ = A ∧
      ∀ (freeEnv : FreeEnv Base) (boundEnv : BoundEnv Γ) {left right : DenoteTy A₀},
        Eval Γ freeEnv boundEnv t₀ A₀ left →
        Eval Γ freeEnv boundEnv u₀ A₀ right → left = right := by
  rcases h with ⟨t₀, u₀, A₀, ⟨equality⟩, rfl, rfl, rfl⟩
  exact ⟨t₀, u₀, A₀, ⟨equality⟩, rfl, rfl, rfl, equality.sound⟩

theorem Proves.sound (h : Proves Γ H p) :
    ∃ H₀ p₀, Nonempty (Nucleus.HolLN.Proves Γ H₀ p₀) ∧
      H₀.map Erasure.toSorted = H ∧ Erasure.toSorted p₀ = p ∧
      Nucleus.HolLN.Entails (Γ := Γ) H₀ p₀ := by
...
theorem HasType.sound (h : HasType Γ t A) :
    ∃ t₀ A₀, Nucleus.HolLN.HasType Γ t₀ A₀ ∧ Erasure.toScoped t₀ = t ∧
      Erasure.toScoped A₀ = A ∧ ∀ (freeEnv : FreeEnv Base) (boundEnv : BoundEnv Γ),
        ∃ value : DenoteTy A₀, Eval Γ freeEnv boundEnv t₀ A₀ value := by
  rcases h with ⟨t₀, A₀, typing, rfl, rfl⟩
  exact ⟨t₀, A₀, typing, rfl, rfl, typing.eval_exists⟩
...
theorem HasType.sound (h : HasType Γ t A) :
    ∃ t₀ A₀, Nucleus.HolLN.HasType Γ t₀ A₀ ∧ Erasure.toRaw t₀ = t ∧
      Erasure.toRaw A₀ = A ∧ ∀ (freeEnv : FreeEnv Base) (boundEnv : BoundEnv Γ),
        ∃ value : DenoteTy A₀, Eval Γ freeEnv boundEnv t₀ A₀ value := by
  rcases h with ⟨t₀, A₀, typing, rfl, rfl⟩
  exact ⟨t₀, A₀, typing, rfl, rfl, typing.eval_exists⟩
