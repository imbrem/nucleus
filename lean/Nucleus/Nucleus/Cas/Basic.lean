...
objects.  A `CasAssertion` is ordinary, unchecked data.  A `CasPair name`
also carries a proof that `name blob = hash`; these checked pairs are the atoms
of a finite `Cas name`.
...
def Valid (name : Bytes → O256) (assertion : CasAssertion) : Prop :=
  name assertion.blob = assertion.hash

instance (name : Bytes → O256) (assertion : CasAssertion) :
    Decidable (assertion.Valid name) := by
  unfold Valid
  infer_instance
...
def hash (pair : CasPair name) : O256 := pair.assertion.hash

/-- The complete blob exposed by a checked pair. -/
def blob (pair : CasPair name) : Bytes := pair.assertion.blob
...
def ofBlob (name : Bytes → O256) (blob : Bytes) : CasPair name where
  assertion := { hash := name blob, blob }
  valid := rfl

@[simp] theorem ofBlob_hash (blob : Bytes) : (ofBlob name blob).hash = name blob := rfl

@[simp] theorem ofBlob_blob (blob : Bytes) : (ofBlob name blob).blob = blob := rfl

/-- Checked pairs are equal when their observable assertions are equal. -/
@[ext]
theorem ext {left right : CasPair name} (equal : left.assertion = right.assertion) :
    left = right := by
  cases left
  cases right
  cases equal
...
-/
def check? (assertion : CasAssertion) (name : Bytes → O256) : Option (CasPair name) :=
  if valid : assertion.Valid name then
    some ⟨assertion, valid⟩
  else
    none

@[simp] theorem check?_isSome (assertion : CasAssertion) :
    (assertion.check? name).isSome = decide (assertion.Valid name) := by
  by_cases valid : assertion.Valid name <;> simp [check?, valid]

theorem check?_sound {assertion : CasAssertion} {pair : CasPair name}
    (checked : assertion.check? name = some pair) :
    pair.assertion = assertion := by
  unfold check? at checked
  split at checked
  · rename_i valid
    have pairEqual : (⟨assertion, valid⟩ : CasPair name) = pair :=
      Option.some.inj checked
    rw [← pairEqual]
  · contradiction

theorem check?_complete {assertion : CasAssertion} (valid : assertion.Valid name) :
    ∃ pair, assertion.check? name = some pair := by
  exact ⟨⟨assertion, valid⟩, by simp [check?, valid]⟩

theorem valid_of_check? {assertion : CasAssertion} {pair : CasPair name}
    (checked : assertion.check? name = some pair) :
    assertion.Valid name := by
...
instance : Membership (CasPair name) (Cas name) :=
  ⟨fun cas pair ↦ pair ∈ cas.pairs⟩

@[simp] theorem mem_iff_mem_pairs {pair : CasPair name} {cas : Cas name} :
    pair ∈ cas ↔ pair ∈ cas.pairs :=
  Iff.rfl

/-- The empty finite CAS. -/
def empty (name : Bytes → O256) : Cas name := ⟨∅⟩

instance : EmptyCollection (Cas name) := ⟨empty name⟩

/-- The CAS containing exactly one checked pair. -/
def singleton (pair : CasPair name) : Cas name := ⟨{pair}⟩

instance : Singleton (CasPair name) (Cas name) := ⟨singleton⟩

/-- Insert a checked pair.  An existing colliding pair is not overwritten. -/
def insert (cas : Cas name) (pair : CasPair name) : Cas name :=
  ⟨{pair} ∪ cas.pairs⟩

instance : Insert (CasPair name) (Cas name) := ⟨fun pair cas ↦ cas.insert pair⟩

/-- Union two finite CAS relations. -/
def union (left right : Cas name) : Cas name :=
  ⟨left.pairs ∪ right.pairs⟩

instance : Union (Cas name) := ⟨union⟩

/-- Keep precisely the checked pairs satisfying `predicate`. -/
def restrict (cas : Cas name) (predicate : CasPair name → Prop)
    [DecidablePred predicate] : Cas name :=
  ⟨cas.pairs.filter predicate⟩

@[simp] theorem not_mem_empty (pair : CasPair name) : pair ∉ (empty name) := by
  simp [empty]

@[simp] theorem mem_singleton {left right : CasPair name} :
    left ∈ singleton right ↔ left = right := by
  simp [singleton]

@[simp] theorem mem_insert {left right : CasPair name} {cas : Cas name} :
    left ∈ cas.insert right ↔ left = right ∨ left ∈ cas := by
  simp [insert]

@[simp] theorem mem_union {pair : CasPair name} {left right : Cas name} :
    pair ∈ left.union right ↔ pair ∈ left ∨ pair ∈ right := by
  simp [union]

@[simp] theorem mem_restrict {pair : CasPair name} {cas : Cas name}
    {predicate : CasPair name → Prop} [DecidablePred predicate] :
    pair ∈ cas.restrict predicate ↔ pair ∈ cas ∧ predicate pair := by
  simp [restrict]

theorem ext_members {left right : Cas name}
    (members : ∀ pair, pair ∈ left ↔ pair ∈ right) : left = right := by
  apply Cas.ext
  exact Finset.ext fun pair ↦ members pair

@[simp] theorem empty_union (cas : Cas name) : (empty name).union cas = cas := by
  apply Cas.ext
  simp [union, empty]

@[simp] theorem union_empty (cas : Cas name) : cas.union (empty name) = cas := by
  apply Cas.ext
  simp [union, empty]

theorem union_comm (left right : Cas name) : left.union right = right.union left := by
  apply Cas.ext
  exact Finset.union_comm left.pairs right.pairs

theorem union_assoc (first second third : Cas name) :
...
@[simp] theorem collisionFree_empty : (empty name).CollisionFree := by
  rw [collisionFree_iff_pairwise]
  intro left _ leftMem
  simp [empty] at leftMem

@[simp] theorem collisionFree_singleton (pair : CasPair name) :
...
def CompatibleWith (cas : Cas name) (pair : CasPair name) : Prop :=
...
theorem compatible_singleton_right {cas : Cas name} {pair : CasPair name} :
...
theorem compatible_singleton_left {cas : Cas name} {pair : CasPair name} :
    (singleton pair).Compatible cas ↔ cas.CompatibleWith pair := by
  rw [compatible_symm, compatible_singleton_right]

theorem compatibleWith_of_mem {cas : Cas name} {pair : CasPair name}
...
theorem collisionFree_insert_iff (cas : Cas name) (pair : CasPair name) :
...
theorem collisionFree_union_iff (left right : Cas name) :
...
theorem hasCollision_mono {small large : Cas name}
...
theorem hasCollision_union_left {left right : Cas name}
...
theorem hasCollision_union_right {left right : Cas name}
...
def Lookup (cas : Cas name) (hash : O256) (blob : Bytes) : Prop :=
  ∃ pair ∈ cas, pair.hash = hash ∧ pair.blob = blob

theorem lookup_of_mem {cas : Cas name} {pair : CasPair name} (member : pair ∈ cas) :
    cas.Lookup pair.hash pair.blob :=
  ⟨pair, member, rfl, rfl⟩

/-- Every lookup result satisfies the naming equation. -/
theorem lookup_valid {cas : Cas name} {hash : O256} {blob : Bytes}
    (found : cas.Lookup hash blob) : name blob = hash := by
...
theorem lookup_functional {cas : Cas name} (collisionFree : cas.CollisionFree)
...
theorem collisionFree_iff_lookup_functional (cas : Cas name) :
    cas.CollisionFree ↔
