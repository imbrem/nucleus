...
test("downloads and attaches an immutable SQLite image in a Worker", async (context) => {
  ...
  assert.equal(result.persistent.kind, "hol-theorem");
  assert.equal(result.persistent.recipe, "truth");
  assert.match(result.persistent.judgement, /^[1-9][0-9]*$/);
  ...
  assert.match(
    await demo.locator("#output").textContent(),
    /judgement\t[1-9][0-9]*/,
  );
});
