...
The underlying `Coverage`, `CoverageDisposition`, and `CoveragePlan` types are
generic schema APIs. Other slices can compose the same declaration/clause/rule
shape with their own case, rejection, and source vocabularies; the add-specific
aliases and builder are only one policy instance.
