...
use crate::snapshot::{SignedSnapshotEnvelope, Signer as _, schema_valid_snapshot_statement};
use crate::{Connection, Kernel};
...
impl<P: Policy> Connection<Prop<P>> {
    ...
    /// schema-qualified envelope under `kernel`'s identity.
    ///
    /// # Errors
    ///
    /// Fails if the policy refuses exports, serialization fails, or the
    /// kernel cannot sign.
    pub fn export_signed(&self, kernel: &Kernel) -> Result<SignedSnapshotEnvelope, PropError> {
    ...
    pub fn import_signed(
        ...
        let _ = self
            ...
            .0
            .sqlite()
            .execute(&format!("DETACH DATABASE \"{IMPORT_SCHEMA}\""), ());
        admitted
    }
...
mod tests {
    ...
    fn signed_export_import_round_trip() {
        ...
        let kernel_one = Kernel::ephemeral();
    ...
    fn tampered_envelopes_and_foreign_signers_are_rejected() {
        let source = certified_php3();
        let kernel_one = Kernel::ephemeral();
        let kernel_two = Kernel::ephemeral();
        let mut receiver = Connection::open_prop_in_memory(AllowAll).expect("open receiver");
        receiver
