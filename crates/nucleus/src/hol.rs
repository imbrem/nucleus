//! HOL-omega prototype with canonical kernel state.
...
/// A kind expression accepted by the representation-independent HOL API.
...
/// One admitted kind-indexed HOL-omega type operator.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum TypeView {
    ...
    /// A free schematic type variable with an explicit kind.
    Free {
        /// Connection-local symbol identity.
        symbol: i64,
        /// Kind assigned to this connection-local atom.
        kind: KindId,
    },
    /// A kind-annotated de Bruijn type occurrence.
    Bound {
        /// Zero-based distance to its enclosing type-operator binder.
        index: u32,
        /// Kind required at this telescope position.
        kind: KindId,
    },
    /// A type-operator abstraction.
    Lambda {
        /// Kind of the newly bound type variable.
        parameter_kind: KindId,
        /// Body, checked beneath the extended kind telescope.
        body: TypeId,
    },
    /// A type-operator application.
    Application {
        /// Operator with an arrow kind.
        operator: TypeId,
        /// Operand with the operator's domain kind.
        argument: TypeId,
    },
    /// A function type.
    Arrow {
...
pub struct TypeInstantiation {
    ...
    pub variable: TypeId,
    /// Locally closed replacement of the key's exact kind, copied without recursive substitution.
...
pub struct UnboundTypeVariable {
    /// Root-relative external index.
    pub index: u32,
    /// Exact kind required at the external telescope position.
    pub kind: KindId,
}
...
impl<P: Policy> Connection<Hol<P>> {
    /// Opens a new in-memory HOL-omega store and installs the current schema.
    ///
    /// # Errors
    ///
    ...
    /// Canonically interns a free schematic type variable at an admitted kind.
    ///
    ...
    pub fn insert_free_type(&mut self, symbol: i64, kind: KindId) -> Result<TypeId, TypeError> {
        let (neutron, hol) = self.parts_mut();
        authorize_type(&mut hol.policy, Operation::InsertType)?;
        read_kind(neutron.sqlite(), kind).map_err(TypeError::Kind)?;
        intern_free_type(neutron.sqlite(), symbol, kind)
    }

    /// Canonically interns a kind-annotated de Bruijn type occurrence.
    ///
    /// # Errors
    ///
    /// Returns an error if policy denies admission or `SQLite` rejects it.
    pub fn insert_bound_type(&mut self, index: u32, kind: KindId) -> Result<TypeId, TypeError> {
        let (neutron, hol) = self.parts_mut();
        authorize_type(&mut hol.policy, Operation::InsertType)?;
        read_kind(neutron.sqlite(), kind).map_err(TypeError::Kind)?;
        intern_bound_type(neutron.sqlite(), index, kind)
    }

    /// Canonically interns a type-operator abstraction.
    ///
    /// # Errors
    ///
    /// Returns an error if policy denies admission, the body is invalid, or insertion fails.
    pub fn insert_type_operator_lambda(
        &mut self,
        parameter_kind: KindId,
        body: TypeId,
        ...
        let (neutron, hol) = self.parts_mut();
        authorize_type(&mut hol.policy, Operation::InsertType)?;
        authorize_type(&mut hol.policy, Operation::InsertKind)?;
        let transaction = neutron.sqlite().unchecked_transaction()?;
        read_kind(&transaction, parameter_kind).map_err(TypeError::Kind)?;
        let body_validation = validate_type(&transaction, body)?;
        require_type_binder(&body_validation.boundary, parameter_kind)?;
        let result =
            intern_type_operator_lambda(&transaction, parameter_kind, body, body_validation.kind)?;
        validate_type(&transaction, result)?;
        transaction.commit()?;
        Ok(result)
    }

    /// Canonically interns a type-operator application after checking exact kinds.
    ///
    /// # Errors
    ///
    /// Returns an error if policy denies admission, the operator has no arrow kind,
    /// the argument kind differs from its domain, or insertion fails.
    pub fn insert_type_operator_application(
        &mut self,
        operator: TypeId,
        argument: TypeId,
    ) -> Result<TypeId, TypeError> {
        ...
        authorize_type(&mut hol.policy, Operation::InsertType)?;
        let transaction = neutron.sqlite().unchecked_transaction()?;
        let operator_validation = validate_type(&transaction, operator)?;
        let argument_validation = validate_type(&transaction, argument)?;
        let KindView::Arrow { domain, codomain } =
            read_kind(&transaction, operator_validation.kind).map_err(TypeError::Kind)?
        else {
            return Err(TypeError::NotOperator(operator_validation.kind));
        };
        if domain != argument_validation.kind {
            return Err(TypeError::ApplicationKindMismatch {
                expected: domain,
                actual: argument_validation.kind,
            });
        }
        let result = intern_type_operator_application(&transaction, operator, argument, codomain)?;
        validate_type(&transaction, result)?;
        transaction.commit()?;
        Ok(result)
    }
    ...
    pub fn insert_arrow_type(
        ...
        authorize_type(&mut hol.policy, Operation::InsertType)?;
        let transaction = neutron.sqlite().unchecked_transaction()?;
        require_star_type(&transaction, domain)?;
        require_star_type(&transaction, codomain)?;
    ...
    pub fn type_kind(&mut self, id: TypeId) -> Result<KindId, TypeError> {
        let (neutron, hol) = self.parts_mut();
        authorize_type(&mut hol.policy, Operation::ReadType)?;
        Ok(validate_type(neutron.sqlite(), id)?.kind)
    }
    ...
    /// Returns external kind-annotated de Bruijn variables required by a type.
    ///
    /// # Errors
    ///
    /// Returns an error if policy denies the read or the type graph is invalid.
    ...
    pub fn insert_free_term(&mut self, symbol: i64, ty: TypeId) -> Result<TermId, TermError> {
        ...
        authorize_term(&mut hol.policy, Operation::InsertTerm)?;
        let transaction = neutron.sqlite().unchecked_transaction()?;
        require_star_type(&transaction, ty)?;
    ...
    pub fn insert_constant(&mut self, symbol: i64, ty: TypeId) -> Result<TermId, TermError> {
        ...
        authorize_term(&mut hol.policy, Operation::InsertTerm)?;
        let transaction = neutron.sqlite().unchecked_transaction()?;
        let validation = require_star_type(&transaction, ty)?;
    ...
    pub fn insert_lambda(
        ...
        let transaction = neutron.sqlite().unchecked_transaction()?;
        require_star_type(&transaction, parameter_type)?;
    ...
    /// Returns external kind-annotated de Bruijn type variables required by a term.
    ///
    /// # Errors
    ///
    /// Returns an error if policy denies the read or the term/type graph is invalid.
...
impl<'brand, P: Policy> ProofSession<'brand, P> {
    ...
    pub fn instantiate_types(
        ...
        for instantiation in instantiations {
            ...
            if !matches!(variable.view, TypeView::Free { .. }) {
            ...
            if variable.kind != replacement.kind {
                return Err(ProofError::TypeInstantiationKindMismatch {
                    variable: instantiation.variable,
                    replacement: instantiation.replacement,
...
fn intern_type_operator_lambda(
    connection: &sqlite::Connection,
    parameter_kind: KindId,
    body: TypeId,
    body_kind: KindId,
) -> Result<TypeId, TypeError> {
    let kind = intern_kind_arrow(connection, parameter_kind, body_kind).map_err(TypeError::Kind)?;
    if let Some(id) = connection
        .query_row(
            "SELECT node_id FROM hol_node
             WHERE tag = 'TLAM' AND lhs = ?1 AND rhs = ?2 AND ty = ?3",
            (parameter_kind.0, body.0, kind.0),
            |row| row.get::<_, i64>(0),
        )
        .optional()?
    {
    ...
    connection.execute(
        "INSERT INTO hol_node(tag, lhs, rhs, ty) VALUES ('TLAM', ?1, ?2, ?3)",
        (parameter_kind.0, body.0, kind.0),
    )?;
    Ok(TypeId(connection.last_insert_rowid()))
}

fn intern_type_operator_application(
    connection: &sqlite::Connection,
    operator: TypeId,
    argument: TypeId,
    kind: KindId,
    ...
    if let Some(id) = connection
        .query_row(
            "SELECT node_id FROM hol_node
             WHERE tag = 'TAPP' AND lhs = ?1 AND rhs = ?2 AND ty = ?3",
            (operator.0, argument.0, kind.0),
            |row| row.get::<_, i64>(0),
        )
        .optional()?
    {
    ...
    connection.execute(
        "INSERT INTO hol_node(tag, lhs, rhs, ty) VALUES ('TAPP', ?1, ?2, ?3)",
        (operator.0, argument.0, kind.0),
    )?;
    Ok(TypeId(connection.last_insert_rowid()))
}
...
fn read_type(connection: &sqlite::Connection, id: TypeId) -> Result<TypeView, TypeError> {
    ...
    match row {
        ...
        (tag, Some(symbol), None, Some(kind)) if tag == "TFV" => Ok(TypeView::Free {
            symbol,
            kind: KindId(kind),
        }),
        (tag, Some(index), None, Some(kind))
            if tag == "TBV" && (0..=i64::from(u32::MAX)).contains(&index) =>
        {
            Ok(TypeView::Bound {
        ...
        (tag, Some(parameter_kind), Some(body), Some(_)) if tag == "TLAM" => Ok(TypeView::Lambda {
            parameter_kind: KindId(parameter_kind),
            body: TypeId(body),
        }),
        (tag, Some(operator), Some(argument), Some(_)) if tag == "TAPP" => {
            Ok(TypeView::Application {
                operator: TypeId(operator),
...
fn validate_type(
    ...
    fn walk(
        ...
        let claimed_kind = read_type_classifier(connection, id)?;
        read_kind(connection, claimed_kind).map_err(TypeError::Kind)?;
        let (kind, boundary) = match view {
            TypeView::Bound { index, kind } => (kind, BTreeMap::from([(index, kind)])),
            TypeView::Free { kind, .. } => (kind, BTreeMap::new()),
            TypeView::Lambda {
                parameter_kind,
                body,
            } => {
                read_kind(connection, parameter_kind).map_err(TypeError::Kind)?;
                let body = walk(connection, body, active, memo)?;
                require_type_binder(&body.boundary, parameter_kind)?;
                match read_kind(connection, claimed_kind).map_err(TypeError::Kind)? {
                    KindView::Arrow { domain, codomain }
                        if domain == parameter_kind && codomain == body.kind => {}
                    _ => return Err(TypeError::CorruptType(id)),
                }
                (
                    claimed_kind,
                    close_type_boundary(body.boundary, parameter_kind)?,
                )
            }
            TypeView::Application { operator, argument } => {
                let operator = walk(connection, operator, active, memo)?;
                let argument = walk(connection, argument, active, memo)?;
                let KindView::Arrow { domain, codomain } =
                    read_kind(connection, operator.kind).map_err(TypeError::Kind)?
                else {
                    return Err(TypeError::NotOperator(operator.kind));
                };
                if domain != argument.kind {
                    return Err(TypeError::ApplicationKindMismatch {
                        expected: domain,
                        actual: argument.kind,
                    });
                }
                if claimed_kind != codomain {
                    return Err(TypeError::CorruptType(id));
                }
                (
                    codomain,
                    merge_type_boundaries(operator.boundary, argument.boundary)?,
                )
            }
            TypeView::Arrow { domain, codomain } => {
                let domain = walk(connection, domain, active, memo)?;
                let codomain = walk(connection, codomain, active, memo)?;
                if claimed_kind != STAR_ID || domain.kind != STAR_ID || codomain.kind != STAR_ID {
                    return Err(TypeError::CorruptType(id));
                }
                (
                    STAR_ID,
                    merge_type_boundaries(domain.boundary, codomain.boundary)?,
                )
            }
            TypeView::Bool | TypeView::Base { .. } => {
                if claimed_kind != STAR_ID {
                    return Err(TypeError::CorruptType(id));
                }
                (STAR_ID, BTreeMap::new())
            }
        };
        active.remove(&id);
...
fn close_type_boundary(
    mut boundary: BTreeMap<u32, KindId>,
    binder: KindId,
) -> Result<BTreeMap<u32, KindId>, TypeError> {
    require_type_binder(&boundary, binder)?;
    boundary.remove(&0);
    Ok(boundary
        .into_iter()
...
fn require_type_binder(boundary: &BTreeMap<u32, KindId>, binder: KindId) -> Result<(), TypeError> {
    if let Some(actual) = boundary.get(&0)
        && *actual != binder
    {
        return Err(TypeError::BinderKindMismatch {
            expected: binder,
            actual: *actual,
        });
    }
...
fn read_type_classifier(connection: &sqlite::Connection, id: TypeId) -> Result<KindId, TypeError> {
    connection
        .query_row(
            "SELECT ty FROM hol_node WHERE node_id = ?1",
            [id.0],
            |row| row.get::<_, Option<i64>>(0),
        )?
        .map(KindId)
        .ok_or(TypeError::CorruptType(id))
}

fn require_star_type(
    connection: &sqlite::Connection,
    id: TypeId,
) -> Result<ValidatedType, TypeError> {
    let validated = validate_type(connection, id)?;
    if validated.kind != STAR_ID {
        return Err(TypeError::ExpectedStar {
            ty: id,
...
fn collect_type_free_variables(
    ...
    fn walk(
        ...
        match read_type(connection, id)? {
            ...
            TypeView::Lambda { body, .. } => {
                walk(connection, body, active, complete, variables)?;
            }
            TypeView::Application { operator, argument } => {
                walk(connection, operator, active, complete, variables)?;
                walk(connection, argument, active, complete, variables)?;
            }
...
fn read_term_node(
    ...
    require_star_type(connection, ty)?;
...
fn instantiate_type_inner(
    ...
    let result = match read_type(connection, ty)? {
        ...
        TypeView::Lambda {
            ...
            let body_kind = validate_type(connection, body)?.kind;
            intern_type_operator_lambda(connection, parameter_kind, body, body_kind)?
        }
        TypeView::Application { operator, argument } => {
            let operator = instantiate_type_inner(connection, operator, replacements, memo)?;
            let argument = instantiate_type_inner(connection, argument, replacements, memo)?;
            let kind = validate_type(connection, ty)?.kind;
            intern_type_operator_application(connection, operator, argument, kind)?
        }
    };
...
pub enum TypeError {
    ...
    CorruptType(TypeId),
    /// A referenced kind is unknown or corrupt.
    Kind(KindError),
    /// A type expected to be an operator has a non-arrow kind.
    NotOperator(KindId),
    /// A type-operator argument has the wrong kind.
    ApplicationKindMismatch {
        /// Operator domain kind.
        expected: KindId,
        /// Argument kind.
        actual: KindId,
    },
    /// The occurrence at the newly bound telescope position has another kind.
    BinderKindMismatch {
        /// Binder's declared kind.
        expected: KindId,
        /// Occurrence's annotation.
        actual: KindId,
    },
    /// An ordinary HOL type position received a higher-kinded operator.
    ExpectedStar {
        /// Rejected type.
        ty: TypeId,
        /// Its actual kind.
...
impl fmt::Display for TypeError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::NotOperator(kind) => {
                write!(formatter, "kind {} is not an operator kind", kind.get())
            }
            Self::ApplicationKindMismatch { expected, actual } => write!(
                formatter,
                "type operator expects kind {}, not {}",
                expected.get(),
                actual.get()
            ),
            Self::BinderKindMismatch { expected, actual } => write!(
                formatter,
                "type binder declares kind {}, but index zero requires {}",
                expected.get(),
                actual.get()
            ),
            Self::ExpectedStar { ty, actual } => write!(
                formatter,
                "type {} has kind {}, not star",
                ty.get(),
                actual.get()
            ),
...
impl StdError for TypeError {
    fn source(&self) -> Option<&(dyn StdError + 'static)> {
        match self {
            ...
            | Self::CorruptType(_)
            | Self::NotOperator(_)
            | Self::ApplicationKindMismatch { .. }
            | Self::BinderKindMismatch { .. }
            | Self::ExpectedStar { .. }
            | Self::InconsistentUnboundVariable { .. } => None,
        }
    }
}
...
pub enum TermError {
    ...
    /// A constant declaration contains an unbound type variable.
...
pub enum ProofError {
    ...
    /// A theorem type-instantiation replacement has a different kind from its key.
    TypeInstantiationKindMismatch {
        variable: TypeId,
        replacement: TypeId,
        expected: KindId,
        actual: KindId,
...
impl fmt::Display for ProofError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            ...
            | Self::TypeInstantiationKindMismatch { .. }
...
fn format_instantiation_proof_error(
    ...
    match error {
        ...
        ProofError::TypeInstantiationKindMismatch {
            variable,
            replacement,
            expected,
            actual,
            ...
            formatter,
            "replacement {} for free type {} has kind {}, expected {}",
            replacement.get(),
            variable.get(),
            actual.get(),
            expected.get()
...
mod tests {
    ...
    #[test]
    fn schematic_star_types_are_canonical_and_constants_remain_monomorphic() {
        let mut connection = Connection::open_hol_in_memory(AllowAll).unwrap();
        let alpha = connection.insert_free_type(-10, STAR_ID).unwrap();
        assert_eq!(connection.insert_free_type(-10, STAR_ID).unwrap(), alpha);
        assert_eq!(
    ...
    fn type_instantiation_is_simultaneous_and_rebuilds_the_exact_context() {
        let mut connection = Connection::open_hol_in_memory(AllowAll).unwrap();
        let alpha = connection.insert_free_type(1000, STAR_ID).unwrap();
        let beta = connection.insert_free_type(1001, STAR_ID).unwrap();
        let bool_type = connection.insert_bool_type().unwrap();
    ...
    fn type_instantiation_rejects_invalid_maps_without_writes() {
        let mut connection = Connection::open_hol_in_memory(AllowAll).unwrap();
        let alpha = connection.insert_free_type(1100, STAR_ID).unwrap();
        let bool_type = connection.insert_bool_type().unwrap();
        let operator_kind = connection
            .insert_kind(&Kind::arrow(Kind::Star, Kind::Star))
            .unwrap();
        let operator = connection.insert_free_type(1102, operator_kind).unwrap();
        ...
        connection
            .with_proof_session(|mut proof| {
                ...
                assert!(matches!(
                    proof.instantiate_types(
                        &theorem,
                        &[TypeInstantiation { variable: alpha, replacement: operator }]
                    ),
                    Err(ProofError::TypeInstantiationKindMismatch {
                        variable,
                        replacement,
                        expected,
                        actual,
    ...
    fn type_instantiation_collapses_exact_free_terms_and_deduplicates_contexts() {
        let mut connection = Connection::open_hol_in_memory(AllowAll).unwrap();
        let alpha = connection.insert_free_type(1200, STAR_ID).unwrap();
        let beta = connection.insert_free_type(1201, STAR_ID).unwrap();
        let bool_type = connection.insert_bool_type().unwrap();
    ...
    fn type_instantiation_checks_all_policy_gates_before_database_work() {
        for denied in [
            ...
            .unwrap();
            let alpha = connection.insert_free_type(1300, STAR_ID).unwrap();
    ...
    fn type_instantiation_rolls_back_partial_syntax_when_context_interning_fails() {
        let mut connection = Connection::open_hol_in_memory(AllowAll).unwrap();
        let alpha = connection.insert_free_type(1400, STAR_ID).unwrap();
        let bool_type = connection.insert_bool_type().unwrap();
    ...
    #[test]
    fn kinded_type_variables_lambdas_and_applications_are_canonical() {
        let mut connection = Connection::open_hol_in_memory(AllowAll).unwrap();
        let operator_kind = connection
            .insert_kind(&Kind::arrow(Kind::Star, Kind::Star))
            .unwrap();
        let operator = connection.insert_free_type(7, operator_kind).unwrap();
        let argument = connection.insert_free_type(8, STAR_ID).unwrap();
        let applied = connection
            .insert_type_operator_application(operator, argument)
            .unwrap();
        let bound_operator = connection.insert_bound_type(0, operator_kind).unwrap();
        let lambda = connection
            .insert_type_operator_lambda(operator_kind, bound_operator)
            .unwrap();

        assert_eq!(connection.type_kind(operator).unwrap(), operator_kind);
        assert_eq!(connection.type_kind(applied).unwrap(), STAR_ID);
        assert_eq!(
            connection.type_view(lambda).unwrap(),
            TypeView::Lambda {
                parameter_kind: operator_kind,
                body: bound_operator,
            }
        );
        assert!(connection.type_is_locally_closed(lambda).unwrap());
        assert_eq!(
            connection
                .insert_type_operator_lambda(operator_kind, bound_operator)
                .unwrap(),
    ...
    #[test]
    fn kinded_type_boundaries_reject_mismatched_binders_and_arguments() {
        let mut connection = Connection::open_hol_in_memory(AllowAll).unwrap();
        let operator_kind = connection
            .insert_kind(&Kind::arrow(Kind::Star, Kind::Star))
            .unwrap();
        let open = connection.insert_bound_type(0, operator_kind).unwrap();
        assert!(matches!(
            connection.insert_type_operator_lambda(STAR_ID, open),
            Err(TypeError::BinderKindMismatch { expected, actual })
                if expected == STAR_ID && actual == operator_kind
        ));

        let operator = connection.insert_free_type(9, operator_kind).unwrap();
        let wrong = connection.insert_free_type(10, operator_kind).unwrap();
        assert!(matches!(
            connection.insert_type_operator_application(operator, wrong),
            Err(TypeError::ApplicationKindMismatch { expected, actual })
                if expected == STAR_ID && actual == operator_kind
        ));
        let bool_type = connection.insert_bool_type().unwrap();
        assert!(matches!(
            connection.insert_type_operator_application(bool_type, wrong),
            Err(TypeError::NotOperator(kind)) if kind == STAR_ID
        ));
        assert!(matches!(
            connection.insert_arrow_type(operator, bool_type),
            Err(TypeError::ExpectedStar { ty, actual })
                if ty == operator && actual == operator_kind
        ));
    }

    #[test]
    fn nested_kinded_telescope_closes_one_exact_position_at_a_time() {
        let mut connection = Connection::open_hol_in_memory(AllowAll).unwrap();
        let operator_kind = connection
            .insert_kind(&Kind::arrow(Kind::Star, Kind::Star))
            .unwrap();
        let inner_operator = connection.insert_bound_type(0, operator_kind).unwrap();
        let outer_argument = connection.insert_bound_type(1, STAR_ID).unwrap();
        let body = connection
            .insert_type_operator_application(inner_operator, outer_argument)
            .unwrap();
        let inner = connection
            .insert_type_operator_lambda(operator_kind, body)
            .unwrap();

        assert_eq!(
            connection.type_unbound_variables(inner).unwrap(),
            [UnboundTypeVariable {
                index: 0,
                kind: STAR_ID,
            }]
        ...
        let closed = connection
            .insert_type_operator_lambda(STAR_ID, inner)
            .unwrap();
        assert!(connection.type_is_locally_closed(closed).unwrap());
    }

    #[test]
    fn erased_type_annotations_keep_boolean_results_type_open() {
        let mut connection = Connection::open_hol_in_memory(AllowAll).unwrap();
        let alpha = connection.insert_bound_type(0, STAR_ID).unwrap();
        let bool_type = connection.insert_bool_type().unwrap();
    ...
    #[test]
    fn constants_reject_open_and_higher_kinded_types() {
        let mut connection = Connection::open_hol_in_memory(AllowAll).unwrap();
        let alpha = connection.insert_bound_type(0, STAR_ID).unwrap();
        assert!(matches!(
        ...
        let schematic = connection.insert_free_type(9_007, STAR_ID).unwrap();
        assert!(matches!(
        ...
        let operator_kind = connection
            .insert_kind(&Kind::arrow(Kind::Star, Kind::Star))
            .unwrap();
        let operator = connection.insert_free_type(9_009, operator_kind).unwrap();
        assert!(matches!(
            connection.insert_constant(9_010, operator),
            Err(TermError::Type(TypeError::ExpectedStar { ty, actual }))
                if ty == operator && actual == operator_kind
        ));
    }
}
