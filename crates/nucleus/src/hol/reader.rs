...
mod tests {
    ...
    #[test]
    fn imported_free_variable_preserves_its_signed_symbol() {
        let view = decode_term(("MFV".to_owned(), Some(-7), None, Some(42)), 9).unwrap();
        let ImportedTermView::Free { symbol, ty } = view else {
            panic!("expected imported free variable")
        };
        assert_eq!(symbol, -7);
        assert_eq!(ty.get(), 42);
    }
    ...
    fn scoped_reader_uses_verified_vfs_and_exposes_only_scoped_evidence() {
        ...
        let negative_free = source.insert_free_term(-79, bool_type).unwrap();
        ...
        matched
            .with_mounted_reader(imported_namespace, &mounted, |mut reader| {
                ...
                let ImportedExport::Term(imported_negative_free) =
                    ...
                    panic!("expected imported negative free-variable export")
                };
                assert_eq!(
