...
pub enum ImportedTypeView<'reader> {
    ...
    Base { symbol: i64 },
    /// Free schematic type variable with its exact kind coordinate.
    Free {
        symbol: i64,
        kind: ImportedKindId<'reader>,
    },
    /// Kind-annotated de Bruijn type occurrence.
    Bound {
        index: u32,
        kind: ImportedKindId<'reader>,
    },
    ...
    Lambda {
        parameter_kind: ImportedKindId<'reader>,
        body: ImportedTypeId<'reader>,
        kind: ImportedKindId<'reader>,
    },
    /// Type-operator application.
    Application {
        operator: ImportedTypeId<'reader>,
...
fn decode_type<'reader>(
    ...
    match row {
        ...
        (tag, Some(symbol), None, Some(kind)) if tag == "TFV" => Ok(ImportedTypeView::Free {
            symbol,
            kind: ImportedKindId(kind, PhantomData),
        }),
        (tag, Some(index), None, Some(kind)) if tag == "TBV" => Ok(ImportedTypeView::Bound {
            index: u32::try_from(index).map_err(|_| ImportedReaderError::CorruptType(id))?,
            kind: ImportedKindId(kind, PhantomData),
        }),
        (tag, Some(parameter_kind), Some(body), Some(kind)) if tag == "TLAM" => {
            Ok(ImportedTypeView::Lambda {
                parameter_kind: ImportedKindId(parameter_kind, PhantomData),
                body: ImportedTypeId(body, PhantomData),
                kind: ImportedKindId(kind, PhantomData),
            })
        }
        (tag, Some(operator), Some(argument), Some(kind)) if tag == "TAPP" => {
            Ok(ImportedTypeView::Application {
                operator: ImportedTypeId(operator, PhantomData),
...
mod tests {
    ...
    use crate::{
        ...
        Kind, NamespaceError, NamespaceExport, SignedSnapshotEnvelope,
    };
    ...
    fn scoped_reader_uses_verified_vfs_and_exposes_only_scoped_evidence() {
        ...
        let negative_free = source.insert_free_term(-7, bool_type).unwrap();
        ...
        let star = source.insert_kind(&Kind::Star).unwrap();
        let operator_kind = source
            .insert_kind(&Kind::arrow(Kind::Star, Kind::Star))
            .unwrap();
        let schematic_type = source.insert_free_type(77, star).unwrap();
        let bound_operator = source.insert_bound_type(0, operator_kind).unwrap();
        let operator_lambda = source
            .insert_type_operator_lambda(operator_kind, bound_operator)
            .unwrap();
        let operator_lambda_kind = source.type_kind(operator_lambda).unwrap();
        let operator = source.insert_free_type(78, operator_kind).unwrap();
        let applied_type = source
        ...
        source
            .export_value(
                namespace,
                ExportId::from_i64(16),
                NamespaceExport::Term(negative_free),
                Some("negative-free"),
            )
            .unwrap();
        source
        ...
        source
            .export_value(
                ...
                NamespaceExport::Type(applied_type),
                Some("applied-type"),
            )
            .unwrap();
        source
            .export_value(
                ...
                NamespaceExport::Type(operator_lambda),
            ...
            .with_mounted_reader(imported_namespace, &mounted, |mut reader| {
                ...
                assert_eq!(
                    ...
                    ImportedTypeView::Free {
                        symbol: 77,
                        kind: ImportedKindId(star.get(), PhantomData),
                    }
                );
                let ImportedExport::Type(imported_lambda) =
                    reader.namespace_export(12).unwrap().unwrap()
                else {
                    panic!("expected imported operator lambda export")
                };
                assert_eq!(
                    reader.type_view(imported_lambda).unwrap(),
                    ImportedTypeView::Lambda {
                        parameter_kind: ImportedKindId(operator_kind.get(), PhantomData),
                        body: ImportedTypeId(bound_operator.get(), PhantomData),
                        kind: ImportedKindId(operator_lambda_kind.get(), PhantomData),
                    }
                );
                let ImportedExport::Type(imported_operator) =
                    reader.namespace_export(14).unwrap().unwrap()
                else {
                    panic!("expected imported operator export")
                };
                assert_eq!(
                    reader.type_view(imported_operator).unwrap(),
                    ImportedTypeView::Free {
                        symbol: 78,
                        kind: ImportedKindId(operator_kind.get(), PhantomData),
                    }
                );
                let ImportedExport::Type(imported_applied) =
                    reader.namespace_export(15).unwrap().unwrap()
                else {
                    panic!("expected imported type application export")
                };
                assert_eq!(
                    reader.type_view(imported_applied).unwrap(),
                    ImportedTypeView::Application {
                        operator: ImportedTypeId(operator.get(), PhantomData),
                        argument: ImportedTypeId(bool_type.get(), PhantomData),
                        kind: ImportedKindId(star.get(), PhantomData),
                    }
                );
                let ImportedExport::Term(imported_negative_free) =
                    reader.namespace_export(16).unwrap().unwrap()
                else {
                    panic!("expected imported negative free term")
                };
                assert_eq!(
                    reader.term(imported_negative_free).unwrap(),
