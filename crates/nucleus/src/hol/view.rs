...
use covalence_lib_sqlite::{self as sqlite};
use covalence_neutron::sql::{self, Param};
...
impl<'v, P: Policy> HolView<'v, P> {
    ...
    pub(crate) fn authorize(&self, operation: Operation) -> Result<(), HolError> {
        if self.connection.policy().allows(operation) {
            Ok(())
        } else {
    ...
    pub(crate) fn insert_object(
        ...
        .context(UnknownIdSnafu { raw: 0_i64 })
    }
    ...
    pub(crate) fn read_object(&self, raw: i64) -> Result<(i64, i64, i64, i64), HolError> {
        sql::query_row(
            self.sqlite(),
            "SELECT tag, lhs, rhs, ty FROM hol_object WHERE id = ?1",
            &[Param::Integer(raw)],
            |row| {
                Ok((
                    row.integer(0)?,
    ...
    pub fn source_from_raw(&self, raw: i64) -> Result<SourceId<'v>, HolError> {
        self.authorize(Operation::ReadSyntax)?;
        let present = sql::query_row(
            self.sqlite(),
            "SELECT source_id FROM hol_source WHERE source_id = ?1",
            &[Param::Integer(raw)],
            |row| row.integer(0),
        )
        .context(StorageSnafu)?;
...
mod tests {
    ...
    fn policy_refusal_blocks_interning() {
        ...
        let connection = Hol::open_in_memory(DenyIntern).expect("open kernel-state database");
        let hol = connection.view();
        assert!(matches!(
            hol.ty(Ty::Bool),
