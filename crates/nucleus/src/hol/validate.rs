...
use super::{
    ...
    validate_term_inner, validate_type,
};
use crate::{
...
const HOL_SEMANTICS: &[u8] = include_bytes!("semantics.txt");

/// Returns the content hash of the current normative HOL semantics.
#[must_use]
pub fn hol_semantics_id() -> O256 {
    o256_path!(::nucleus.hol.protocol.semantics).tag(HOL_SEMANTICS)
}

/// Derives the current signed schema identity from semantics and exact physical bytes.
#[must_use]
pub fn hol_schema_id(physical_schema: O256) -> O256 {
    let mut commitments = [0_u8; 64];
    commitments[..32].copy_from_slice(hol_semantics_id().as_ref());
    commitments[32..].copy_from_slice(physical_schema.as_ref());
    o256_path!(::nucleus.hol.protocol.sqlite_schema).tag(commitments)
}
...
impl ValidatedHolImage {
    ...
    pub fn validate_with_schema(
        ...
        let schema = hol_schema_id(physical_schema);
...
fn validate_schema(
    ...
    let representation =
        connection.query_row("SELECT representation FROM hol_schema", [], |row| {
            row.get::<_, String>(0)
        })?;
    if representation == "tagged-node" {
...
pub(super) fn expected_composite_schema_id(
    ...
    expected.sqlite().execute_batch(SCHEMA)?;
    install_metadata_schema(expected.sqlite(), schema)?;
    Ok(hol_schema_id(schema_manifest_id(&schema_manifest(
        expected.sqlite(),
    )?)))
}
...
fn validate_contents(
    ...
    for (id, tag, _, _, _) in &nodes {
        match tag.as_bytes().first() {
            ...
            Some(b'T') => {
                validate_type(connection, TypeId(*id))?;
            }
            Some(b'M') => {
...
pub enum HolImageValidationError {
    ...
    /// Imported aliases cannot contain local child paths.
...
mod tests {
    ...
    #[test]
    fn semantic_schema_identity_is_stable() {
        ...
        assert_eq!(
            physical,
            O256::from_hex("8019cd0902e2aa7497af77743ee8878187fa616727393b7b8fdc480b4fcc2361")
                .unwrap()
        );
        assert_eq!(
            hol_semantics_id(),
            O256::from_hex("335dd0aee30fba0cb7a5c53f993b934caae24f9659cb21653750a49e0eea6e66")
                .unwrap()
        );
        assert_eq!(
            hol_schema_id(physical),
            O256::from_hex("b83f052cace4477fdc357926f27dea04cb232c9f267bf86e1b310c36e00b30cb")
                .unwrap()
        );
    ...
    fn validates_exact_owned_bytes_without_trusting_judgement_truth() {
        ...
        assert_eq!(validated.schema(), hol_schema_id(physical));
        assert_ne!(validated.schema(), validated.physical_schema());
        assert_ne!(validated.schema(), hol_semantics_id());
    ...
    #[test]
    fn detached_validation_rejects_direct_and_nested_schematic_constants() {
        let mut connection = Connection::open_hol_in_memory(AllowAll).unwrap();
        let alpha = connection.insert_free_type(501, super::STAR_ID).unwrap();
        let bool_type = connection.insert_bool_type().unwrap();
        let nested = connection.insert_arrow_type(bool_type, alpha).unwrap();
        let bytes = connection.parts_mut().0.serialize().unwrap();
        ...
        for (symbol, ty) in [(601_i64, alpha), (602, nested)] {
            ...
            corrupt
                ...
                .unwrap();
            assert!(matches!(
                ValidatedHolImage::validate(&corrupt.serialize().unwrap()),
                Err(HolImageValidationError::Term(
                    TermError::PolymorphicConstantType { symbol: actual, ty: actual_ty }
                )) if actual == symbol && actual_ty == ty
            ));
        }
    }

    #[test]
    fn detached_validation_rechecks_malformed_and_cyclic_epsilon_nodes() {
        let mut connection = Connection::open_hol_in_memory(AllowAll).unwrap();
        let bool_type = connection.insert_bool_type().unwrap();
        ...
        let wrong_type = covalence_neutron::Connection::deserialize(&bytes).unwrap();
        wrong_type
            .sqlite()
            .execute(
                "UPDATE hol_node SET ty = ?1 WHERE node_id = ?2",
                [
                    connection.term_type(predicate).unwrap().get(),
                    epsilon.get(),
                ],
            )
            .unwrap();
        assert!(matches!(
            ValidatedHolImage::validate(&wrong_type.serialize().unwrap()),
            Err(HolImageValidationError::Term(TermError::CorruptTerm(term)))
                if term == epsilon
        ));

        let wrong_predicate = covalence_neutron::Connection::deserialize(&bytes).unwrap();
        wrong_predicate
            .sqlite()
            .execute(
                "UPDATE hol_node SET lhs = ?1 WHERE node_id = ?2",
                [bound.get(), epsilon.get()],
            )
            .unwrap();
        assert!(matches!(
            ValidatedHolImage::validate(&wrong_predicate.serialize().unwrap()),
            Err(HolImageValidationError::Term(TermError::NotFunction(ty)))
                if ty == bool_type
        ));

        let cyclic = covalence_neutron::Connection::deserialize(&bytes).unwrap();
        cyclic
            .sqlite()
            .execute(
                "UPDATE hol_node SET lhs = ?1 WHERE node_id = ?1",
                [epsilon.get()],
            )
            .unwrap();
        assert!(matches!(
            ValidatedHolImage::validate(&cyclic.serialize().unwrap()),
            Err(HolImageValidationError::CyclicNodeGraph(node)) if node == epsilon.get()
        ));
    }

    #[test]
    fn detached_validation_rechecks_kinded_type_operators() {
        let mut connection = Connection::open_hol_in_memory(AllowAll).unwrap();
        let star = connection.insert_kind(&super::super::Kind::Star).unwrap();
        let operator_kind = connection
            .insert_kind(&super::super::Kind::arrow(
                super::super::Kind::Star,
                super::super::Kind::Star,
            ))
            .unwrap();
        let bool_type = connection.insert_bool_type().unwrap();
        let operator = connection.insert_free_type(701, operator_kind).unwrap();
        let application = connection
            .insert_type_operator_application(operator, bool_type)
            .unwrap();
        let bound = connection.insert_bound_type(0, operator_kind).unwrap();
        let lambda = connection
            .insert_type_operator_lambda(operator_kind, bound)
            .unwrap();
        let bytes = connection.parts_mut().0.serialize().unwrap();

        let wrong_binder = covalence_neutron::Connection::deserialize(&bytes).unwrap();
        wrong_binder
            .sqlite()
            .execute(
                "UPDATE hol_node SET lhs = ?1 WHERE node_id = ?2",
            ...
            .unwrap();
        let result = ValidatedHolImage::validate(&wrong_binder.serialize().unwrap());
        let error = result.err().expect("corrupt binder must fail validation");
        assert!(
            matches!(
                &error,
                HolImageValidationError::Type(TypeError::BinderKindMismatch {
                    expected,
                    actual,
                }) if *expected == star && *actual == operator_kind
            ),
            "{error:?}"
        );

        let wrong_argument = covalence_neutron::Connection::deserialize(&bytes).unwrap();
        wrong_argument
            .sqlite()
            .execute(
                "UPDATE hol_node SET rhs = ?1 WHERE node_id = ?2",
                [operator.get(), application.get()],
            )
            .unwrap();
        assert!(matches!(
            ValidatedHolImage::validate(&wrong_argument.serialize().unwrap()),
            Err(HolImageValidationError::Type(TypeError::ApplicationKindMismatch {
                expected,
                actual,
            })) if expected == star && actual == operator_kind
        ));

        let wrong_classifier = covalence_neutron::Connection::deserialize(&bytes).unwrap();
        wrong_classifier
            .sqlite()
            .execute(
                "UPDATE hol_node SET ty = ?1 WHERE node_id = ?2",
                [operator_kind.get(), application.get()],
            )
            .unwrap();
        assert!(matches!(
            ValidatedHolImage::validate(&wrong_classifier.serialize().unwrap()),
            Err(HolImageValidationError::Type(TypeError::CorruptType(ty)))
                if ty == application
        ));

        let higher_kinded_term = covalence_neutron::Connection::deserialize(&bytes).unwrap();
        higher_kinded_term
            .sqlite()
            .execute(
                "INSERT INTO hol_node(tag, lhs, ty) VALUES ('MFV', 702, ?1)",
                [operator.get()],
            )
            .unwrap();
        assert!(matches!(
            ValidatedHolImage::validate(&higher_kinded_term.serialize().unwrap()),
            Err(HolImageValidationError::Term(TermError::Type(TypeError::ExpectedStar {
                ty,
                actual,
    ...
    fn physical_schema_rejects_null_payloads_and_moved_primitives() {
        ...
        assert!(
            connection
                .sqlite()
                .execute(
                    "INSERT INTO hol_node(tag, lhs, rhs, ty) VALUES ('MEPS', 1, 1, 2)",
                    [],
                )
                .is_err()
        );
    ...
    fn validates_only_the_exact_declared_metadata_schema() {
        ...
        assert_eq!(
            ...
            hol_schema_id(validated.physical_schema_manifest())
        );
