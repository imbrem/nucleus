...
mod tests {
    ...
    #[test]
    fn reserved_core_indexes_match_the_physical_schema() {
        let schema_indexes = include_str!("schema.sql")
            .lines()
            .filter_map(|line| line.trim().strip_prefix("CREATE UNIQUE INDEX "))
            .collect::<HashSet<_>>();
        let reserved_indexes = CORE_INDEX_NAMES.iter().copied().collect::<HashSet<_>>();

        assert_eq!(reserved_indexes, schema_indexes);
    }
    ...
    fn mixed_descriptor_has_fixed_wire_and_composite_vectors() {
        ...
        assert_eq!(
            ...
            "20bde166eb8444a158d41e001651db4badc7ea20a9fcec6b8144f1321e17d069"
        );
    }
