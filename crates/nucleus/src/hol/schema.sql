...
CREATE TABLE hol_node (
    ...
    CHECK (COALESCE((
        ...
        OR (tag = 'TFV' AND lhs IS NOT NULL AND rhs IS NULL AND ty IS NOT NULL)
        OR (
            tag = 'TBV'
        ...
        OR (tag = 'TLAM' AND lhs IS NOT NULL AND rhs IS NOT NULL AND ty IS NOT NULL)
        OR (tag = 'TAPP' AND lhs IS NOT NULL AND rhs IS NOT NULL AND ty IS NOT NULL)
...
CREATE UNIQUE INDEX hol_tlam_unique
    ON hol_node(lhs, rhs) WHERE tag = 'TLAM';

CREATE UNIQUE INDEX hol_tapp_unique
    ON hol_node(lhs, rhs) WHERE tag = 'TAPP';
