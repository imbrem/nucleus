...
use covalence_neutron::sql::{self, Param, Transaction};
...
impl<'v, P: Policy> HolView<'v, P> {
    ...
    pub fn proof_step<R: Rule<'v>>(&self, rule: R) -> Result<R::Output, HolError> {
        ...
        // All of a rule or none of it: a premise that fails halfway must
        // leave no interned syntax behind, or the store would accumulate
        // fragments no theorem refers to. Dropping without committing rolls
        // back, which is what the `?` below rely on.
        let transaction = Transaction::begin(self.sqlite()).context(StorageSnafu)?;
    ...
    fn insert_theorem(
        ...
        sql::query_row(
            ...
            &[
                Param::Integer(kinds.raw()),
                Param::Integer(vars.raw()),
                Param::Integer(hyps.raw()),
                Param::Integer(concl.raw()),
            ],
            |row| row.integer(0),
        )
        .context(StorageSnafu)?
        .map(TheoremId::new)
        .ok_or_else(|| UnknownIdSnafu { raw: 0_i64 }.build())
    }
