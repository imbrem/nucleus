...
impl<P: Policy> Connection<Prop<P>> {
    ...
    pub fn schema_id(&self) -> Result<O256, PropError> {
        let physical =
            crate::manifest::schema_manifest_id(self.parts().0.sqlite()).context(StorageSnafu)?;
        Ok(prop_schema_id(physical))
    }
}

impl crate::Kernel {
    /// Opens a fresh in-memory propositional connection under `policy`.
    ///
    /// # Errors
    ///
    /// Returns an error if the connection cannot be opened or the schema
    /// cannot be installed.
    pub fn open_prop<Q: Policy>(&self, policy: Q) -> Result<Connection<Prop<Q>>, PropError> {
        Connection::open_prop_in_memory(policy)
    }
}
...
impl<P: Policy> PropView<'_, P> {
    ...
    fn insert_row(&self, lhs: i64, rhs: i64, model: i64) -> Result<i64, PropError> {
        self.sqlite()
            ...
            .and_then(|mut statement| {
                statement.query_row((lhs, rhs, model), |row| row.get::<_, i64>(0))
            })
            .context(StorageSnafu)
    }
    ...
    fn usable(&self, lhs: i64, rhs: i64, target: Target) -> Result<bool, PropError> {
        self.sqlite()
            ...
            .and_then(|mut statement| {
                statement
                    .query_row((lhs, rhs, target.world_model()), |_| Ok(()))
                    .optional()
            })
            .context(StorageSnafu)
    ...
    fn require_world(&self, world: WorldId) -> Result<(), PropError> {
        let present: Option<i64> = self
            .sqlite()
            .prepare_cached("SELECT world_id FROM prop_world WHERE world_id = ?1")
            .and_then(|mut statement| {
                statement
                    .query_row((world.get(),), |row| row.get(0))
                    .optional()
            })
            .context(StorageSnafu)?;
        if present.is_some() {
    ...
    fn require_undetermined(&self, id: PropId) -> Result<(), PropError> {
        let touched: Option<i64> = self
            .sqlite()
            .prepare_cached("SELECT lhs FROM prop_row WHERE lhs = ?1 OR lhs = ?2 LIMIT 1")
            .and_then(|mut statement| {
                statement
                    .query_row((id.get(), -id.get()), |row| row.get(0))
                    .optional()
            })
            .context(StorageSnafu)?;
        if touched.is_some() {
    ...
    fn is_free(&self, id: PropId) -> Result<bool, PropError> {
        ...
        // theory-bound.
        let binding: Option<(i64, i64)> = self
            .sqlite()
            .prepare_cached(
            ...
            .and_then(|mut statement| {
                statement
                    .query_row((id.get(),), |row| Ok((row.get(0)?, row.get(1)?)))
                    .optional()
            })
            .context(StorageSnafu)?;
        Ok(match binding {
    ...
    pub fn world(&self, meaning: Option<&str>) -> Result<WorldId, PropError> {
        ...
        self.sqlite()
            .prepare_cached("INSERT INTO prop_world(meaning) VALUES (?1) RETURNING world_id")
    ...
    pub fn define(&self, id: PropId, conjuncts: &[Lit]) -> Result<(), PropError> {
        ...
        for conjunct in conjuncts {
            ...
            let reaches: Option<i64> = self
                ...
                .and_then(|mut statement| {
                    statement
                        .query_row((conjunct.proposition().get(), id.get()), |row| row.get(0))
                        .optional()
                })
                .context(StorageSnafu)?;
            if reaches.is_some() {
        ...
        for conjunct in conjuncts {
            transaction
                ...
                .and_then(|mut statement| statement.execute((id.get(), conjunct.get())))
                .context(StorageSnafu)?;
        }
    ...
    pub(crate) fn check_validity_in(&self, schema: &str) -> Result<Vec<String>, PropError> {
        ...
        let mut collect = |sql: &str, label: &str| -> Result<(), PropError> {
            let mut statement = sqlite.prepare(sql).context(StorageSnafu)?;
            let rows = statement
                .query_map((), |row| row.get::<_, i64>(0))
                .context(StorageSnafu)?;
            for row in rows {
                let value = row.context(StorageSnafu)?;
                violations.push(format!("{label}: {value}"));
            }
            Ok(())
        };
