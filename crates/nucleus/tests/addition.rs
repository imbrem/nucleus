...
#[test]
fn supports_connection_local_addition_relations() {
    let connection = Connection::open_in_memory().unwrap();
    let catalog = connection.catalog("temp").unwrap();
    let mut addition = catalog.create_addition("scratch_addition").unwrap();
    let theorem = AdditionFact::sum(20, 22).unwrap();
    addition.insert(&theorem).unwrap();
    assert_eq!(addition.facts().unwrap(), [theorem]);
}
