//! Untrusted component which requests the closed-eta demo proof.
//!
...
mod component {
    ...
    struct ClosedEtaGuest;

    impl Guest for ClosedEtaGuest {
        fn build(
            plan: &ProofPlan,
            ...
            plan.persist_theorem(&theorem)
                .map_err(|_| GuestError::Aborted)?;
            let namespace = plan
                .root_child_namespace(Some("eta-demo"))
                .map_err(|_| GuestError::Aborted)?;
            plan.export_context(&namespace, 0, &context, Some("empty_context"))
                .map_err(|_| GuestError::Aborted)?;
            plan.export_theorem_conclusion(&namespace, 1, &theorem, Some("identity_eta"))
                .map_err(|_| GuestError::Aborted)?;
            Ok(namespace)
        }
    }
    ...
    mod component_export {
        use super::{ClosedEtaGuest, bindings};

        bindings::export!(ClosedEtaGuest with_types_in bindings);
    }
}
