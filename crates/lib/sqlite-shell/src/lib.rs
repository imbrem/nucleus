//! The upstream `SQLite` command-line shell, embedded in this process.
//!
...
//! same `SQLite` library the rest of the workspace uses.
//!
//! Sharing one library is the point. `SQLite`'s VFS registry is process-global,
//! so a VFS registered from Rust — a mounted CAS, say — is already visible to
//! the shell. There is no bridge to build and no second registry to keep in
//! sync: `.open 'file:<address>?vfs=cas'` works inside the shell because the
//! shell and the host are the same `SQLite`.
//!
//! # Trust
//!
//! The shell is outside the trusted computing base. It runs arbitrary SQL
//! against whatever connection it is pointed at, which is exactly what makes
//! it useful for exploration and exactly why nothing may depend on it for a
//! correctness claim. It is compiled without extension loading and without
//! `system()`, so it cannot acquire capabilities the host process did not
//! already intend to give it.
