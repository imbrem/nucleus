...
mod tests {
    ...
    fn an_empty_buffer_derefs_to_an_empty_slice() {
        ...
        let bytes = unsafe { Bytes::from_raw(std::ptr::null_mut(), 0) };
    ...
    fn bytes_from_sqlite_deref_and_free() {
        ...
        let bytes = unsafe { Bytes::from_raw(raw, 4) };
    ...
    fn a_copy_owns_its_own_allocation() {
        let bytes = Bytes::copy_from_slice(b"abcd").expect("allocate");
        assert_eq!(&*bytes, b"abcd");
        assert!(Bytes::copy_from_slice(b"").expect("allocate").is_empty());
    }
    ...
    fn into_raw_gives_up_ownership() {
        ...
        // SAFETY: adopting exactly that allocation.
        let bytes = unsafe { Bytes::from_raw(raw, 1) };
