...
impl<V: Vfs + 'static> SQLiteVfs<Io<V>> for VfsAdapter<V> {
    ...
    fn random(buf: &mut [u8]) {
        // Minimal: fill with zeros. Callers who need real randomness should
        // provide their own VFS or override at the SQLite level.
        buf.fill(0);
    }
...
/// # Errors
///
/// Returns an error if the name contains a NUL byte or if
/// `sqlite3_vfs_register` fails.
