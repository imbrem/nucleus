//! The upstream `SQLite` shell as a WebAssembly component.
...
/// # Errors
///
/// Returns [`ShellError`] when an argument cannot be passed to C.
pub fn run<S: AsRef<str>>(arguments: &[S]) -> Result<i32, ShellError> {
    ...
    // SAFETY: argv is NUL-terminated and `owned` keeps its strings alive.
