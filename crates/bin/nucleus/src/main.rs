...
.shell [ARG...]    open the real SQLite shell with the store mounted
                   a bare ADDRESS argument expands to its file: URI
.help              show this
.quit              leave
";
...
/// Hands off to the real `SQLite` shell.
///
/// The store is already mounted process-globally, so the shell needs nothing
/// from us but its arguments. A bare address is expanded to the URI which
/// opens it, because typing the full `file:...?vfs=cas` form every time is
...
fn shell(repl: &Repl, rest: &str) -> Result<i32, Box<dyn std::error::Error>> {
    let arguments: Vec<String> = split_arguments(rest)?
        ...
        .collect();
    Ok(covalence_lib_sqlite_shell::run(&arguments)?)
}
