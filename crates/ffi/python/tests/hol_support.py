...
def reflexive_children(kernel: Kernel, reference: int) -> list[int]:
    ...
    return [kernel.syn_refl("syn", child).id for child in definition.children]
...
def _kernel_reference_calls() -> list[tuple[str, Callable[[Kernel], object]]]:
    ...
    return [
        ...
        ("syn_congr.child", lambda kernel: kernel.syn_congr("syn", 1, 1, [0])),
