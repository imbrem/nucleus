...
// Independently pinned below after the checked constructor is rendered. The
// literal deliberately contains no primitive `false` leaf: logical false is
// `ALL_B (lambda p:bool. p)` throughout this alternative fixture.
const DEDEKIND_INFINITY_GRAPH: &str = r"(APP (LAM:(I->I) (APP (APP (LAM:B (LAM:B (EQ (LAM:(B->(B->B)) (APP (APP #0:(B->(B->B)) #2:B) #1:B)) (LAM:(B->(B->B)) (APP (APP #0:(B->(B->B)) true) true))))) (APP (LAM:(I->I) (EQ (LAM:I (EQ (LAM:I (EQ (APP (APP (LAM:B (LAM:B (EQ (LAM:(B->(B->B)) (APP (APP #0:(B->(B->B)) #2:B) #1:B)) (LAM:(B->(B->B)) (APP (APP #0:(B->(B->B)) true) true))))) (EQ (APP #2:(I->I) #1:I) (APP #2:(I->I) #0:I))) (EQ #1:I #0:I)) (EQ (APP #2:(I->I) #1:I) (APP #2:(I->I) #0:I)))) (LAM:I true))) (LAM:I true))) #0:(I->I))) (EQ (APP (LAM:(I->I) (EQ (LAM:I (APP (LAM:I (EQ (APP #2:(I->I) #0:I) #1:I)) (EPS (LAM:I (EQ (APP #2:(I->I) #0:I) #1:I))))) (LAM:I true))) #0:(I->I)) (EQ (LAM:B #0:B) (LAM:B true))))) (EPS (LAM:(I->I) (APP (APP (LAM:B (LAM:B (EQ (LAM:(B->(B->B)) (APP (APP #0:(B->(B->B)) #2:B) #1:B)) (LAM:(B->(B->B)) (APP (APP #0:(B->(B->B)) true) true))))) (APP (LAM:(I->I) (EQ (LAM:I (EQ (LAM:I (EQ (APP (APP (LAM:B (LAM:B (EQ (LAM:(B->(B->B)) (APP (APP #0:(B->(B->B)) #2:B) #1:B)) (LAM:(B->(B->B)) (APP (APP #0:(B->(B->B)) true) true))))) (EQ (APP #2:(I->I) #1:I) (APP #2:(I->I) #0:I))) (EQ #1:I #0:I)) (EQ (APP #2:(I->I) #1:I) (APP #2:(I->I) #0:I)))) (LAM:I true))) (LAM:I true))) #0:(I->I))) (EQ (APP (LAM:(I->I) (EQ (LAM:I (APP (LAM:I (EQ (APP #2:(I->I) #0:I) #1:I)) (EPS (LAM:I (EQ (APP #2:(I->I) #0:I) #1:I))))) (LAM:I true))) #0:(I->I)) (EQ (LAM:B #0:B) (LAM:B true)))))))";
...
/// Builds the canonical, definitional logical false `ALL_B (lambda p:bool. p)`.
///
/// This is syntax only. It introduces no judgement and uses no primitive
/// Boolean-false term. Under this fixture's equality-based encoding,
/// `ALL_B P` is `P = (lambda _:bool. true)`.
///
/// # Errors
///
/// Returns if a checked HOL syntax constructor rejects the exact graph.
pub fn build_canonical_false(
    connection: &mut Connection<Hol<AllowAll>>,
) -> Result<TermId, SignedHolRoundTripError> {
    let bool_type = connection
        .insert_bool_type()
        .map_err(|error| SignedHolRoundTripError::at("canonical-false-syntax-checked", error))?;
    ...
    let identity = connection
        .insert_lambda(bool_type, proposition)
        .map_err(|error| SignedHolRoundTripError::at("canonical-false-syntax-checked", error))?;
    logical_all(connection, bool_type, identity, truth)
        .map_err(|error| SignedHolRoundTripError::at("canonical-false-syntax-checked", error))
}

fn ensure_no_primitive_false_reachable(
    connection: &mut Connection<Hol<AllowAll>>,
    root: TermId,
    ...
    fn walk(
        ...
        {
            TermView::Bool(false) => Err(SignedHolRoundTripError::invalid(
                "assumption-profile-checked",
                "Dedekind-infinity graph reaches primitive Boolean false",
            )),
            ...
            | TermView::Equality {
                ...
                walk(connection, function, visited)?;
                walk(connection, argument, visited)
            }
            TermView::Lambda { body, .. }
            | TermView::Epsilon { predicate: body }
            | TermView::TypeLambda { body } => walk(connection, body, visited),
            TermView::TypeApplication { function, .. } => walk(connection, function, visited),
            TermView::Bool(true)
            | TermView::Free { .. }
            | TermView::Constant { .. }
            | TermView::Bound { .. } => Ok(()),
    ...
    walk(connection, root, &mut HashSet::new())
}
...
pub fn build_dedekind_infinity_syntax(
    ...
    ensure_no_primitive_false_reachable(connection, conclusion)?;
...
fn verify_raw_assumption_profile(
    ...
    if primitive_false_rows != 0 {
        return Err(SignedHolRoundTripError::invalid(
            "assumption-profile-checked",
            "fixture database contains primitive Boolean false",
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn canonical_false_is_definitional_and_uses_no_primitive_false() {
        let mut connection = Connection::open_hol_in_memory(AllowAll).unwrap();
        let falsehood = build_canonical_false(&mut connection).unwrap();
        assert_eq!(
            render_term(&mut connection, falsehood).unwrap(),
            "(EQ (LAM:B #0:B) (LAM:B true))"
        );
        ensure_no_primitive_false_reachable(&mut connection, falsehood).unwrap();
    }

    #[test]
    fn raw_profile_rejects_even_unreachable_primitive_false() {
        let kernel = Kernel::ephemeral();
        let mut checked = kernel.open_hol(AllowAll).unwrap();
        let syntax = build_dedekind_infinity_syntax(&mut checked).unwrap();
        let namespace = export_assumption_namespace(&mut checked, syntax).unwrap();
        let checked_image = kernel.export_hol(&mut checked).unwrap();
        let raw = covalence_neutron::Connection::deserialize(
            &covalence_neutron::Bytes::copy_from_slice(checked_image.image().bytes()),
        )
        .unwrap();
        verify_raw_assumption_profile(&raw, namespace, syntax).unwrap();
        ...
        raw.sqlite()
            ...
            .unwrap();
        let error = verify_raw_assumption_profile(&raw, namespace, syntax).unwrap_err();
