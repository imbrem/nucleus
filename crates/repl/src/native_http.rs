...
//! capabilities, and session closure remain properties of [`SignedKernelService`].
//! This prototype binds loopback only. Non-loopback or federated deployment
//! requires an explicit requester-key authorization policy.
...
pub const MAX_NATIVE_HTTP_REQUESTS: usize = 64;
...
impl NativeHttpKernelServer {
    ...
    /// Serves connections for the caller-owned server lifetime.
    ///
    ...
    pub fn serve(mut self) -> Result<(), NativeHttpError> {
        loop {
            ...
            match self.serve_stream(&mut stream) {
                Ok(session_closed) => {
                    if session_closed && self.stop_on_session_close {
                        return Ok(());
                    }
                }
                Err(error) => {
    ...
    fn serve_stream(&mut self, stream: &mut TcpStream) -> Result<bool, NativeHttpError> {
        ...
        let (response, session_closed) = match message {
            ...
            SignedMessageRequest::Execute(command) => {
                let reply = self
                    ...
                    .execute(&command)
                    .map_err(|error| NativeHttpError::Service(error.to_string()))?;
                let session_closed = reply.is_session_closed();
                (SignedMessageResponse::Reply(reply), session_closed)
            }
        };
...
mod tests {
    ...
    fn spawn_server() -> (SocketAddr, JoinHandle<Result<(), NativeHttpError>>) {
        let mut server = NativeHttpKernelServer::bind(
            (std::net::Ipv4Addr::LOCALHOST, 0),
            "https://repl.example",
        )
        .unwrap();
        // Tests own this private carrier and request deterministic termination;
        // production callers retain server lifetime independently of sessions.
        server.stop_on_session_close = true;
    ...
    fn close_session(
        ...
        let command = session.command(ServiceOperation::CloseSession).unwrap();
    ...
    fn native_client_pins_and_drives_the_same_signed_service() {
        ...
        assert!(matches!(
            client.execute(ServiceOperation::CloseSession).unwrap(),
            ServiceResult::SessionClosed
        ));
    ...
    fn native_client_rejects_the_http_description_against_the_out_of_band_key() {
        ...
        close_session(address, session, handle);
    }
    ...
    #[test]
    fn serves_strict_cors_and_authenticates_session_close() {
        let (address, handle) = spawn_server();
        let options = format!(
        ...
        let (session, _) = open_session(address);
        let wrong_path = b"POST /stop-process HTTP/1.1\r\nHost: localhost\r\nContent-Type: application/octet-stream\r\nContent-Length: 0\r\n\r\n";
    ...
    fn accepts_only_exact_http_origins() {
        ...
        assert!(matches!(
            NativeHttpKernelServer::bind(
                (std::net::Ipv4Addr::UNSPECIFIED, 0),
                "https://repl.example"
            ),
            Err(NativeHttpError::Protocol(
                "native HTTP MVP binds only loopback addresses"
            ))
        ));
    }
    ...
    fn tampered_command_gets_signed_rejection_without_advancing_session() {
        ...
        *body.last_mut().unwrap() ^= 1;
        ...
        let SignedMessageResponse::Reply(reply) =
            ...
            panic!("expected signed session-close reply")
        };
        assert!(matches!(
            session.accept_reply(&command, reply).unwrap(),
            ServiceResult::SessionClosed
