...
impl Definitions {
    ...
    pub fn prepare_all_elim<P: Policy>(
        ...
        if !connection.term_is_locally_closed(argument)? {
            return Err(DerivedError::OpenArgument(argument));
        }
...
pub enum DerivedError {
    ...
    /// Universal elimination requires a closed argument.
    OpenArgument(TermId),
}
...
impl fmt::Display for DerivedError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::OpenArgument(argument) => write!(
                formatter,
                "universal-elimination argument {} is not locally closed",
                argument.get()
            ),
        }
    }
...
impl StdError for DerivedError {
    fn source(&self) -> Option<&(dyn StdError + 'static)> {
        match self {
            ...
            Self::OpenPredicate(_) | Self::OpenArgument(_) => None,
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn checked_all_elim_rejects_an_open_argument_during_preparation() {
        let mut connection = Connection::open_hol_in_memory(AllowAll).unwrap();
        let definitions = Definitions::new(&mut connection).unwrap();
        let bool_type = definitions.bool_type();
        let bound = connection.insert_bound_term(0, bool_type).unwrap();
        let identity = connection.insert_lambda(bool_type, bound).unwrap();
        let open_argument = connection.insert_bound_term(0, bool_type).unwrap();

        let error = definitions
            .prepare_all_elim(&mut connection, bool_type, identity, open_argument)
            .unwrap_err();
        assert!(matches!(
            error,
            DerivedError::OpenArgument(term) if term == open_argument
        ));
    }
