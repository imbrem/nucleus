...
const MISSING_ORACLE: &str =
    "(LAM:I (EQ (APP (LAM:I (EQ (APP s #0:I) #1:I)) (EPS (LAM:I (EQ (APP s #0:I) #1:I)))) false))";
...
pub fn build_natlike_syntax(
    ...
    verify_exact_natlike_oracle(connection, ind, succ, conjunction, syntax)?;
    Ok(syntax)
}
...
pub fn produce_signed_natlike_artifact(
    ...
    let syntax = build_natlike_syntax(&mut connection, inherited.ind, inherited.successor)?;
...
mod tests {
    ...
    fn signed_natlike_extension_has_one_inherited_judgement_and_reopens() {
        ...
        assert!(
            artifact
                .attestation_text()
                .starts_with("authority=signed-assumption\nassumption=dedekind-infinity\n")
        );
        let child = directory
