...
const Q_ORACLE: &str = "(EQ (APP (LAM:(I->I) (EQ (LAM:I (APP (LAM:I (EQ (APP #2:(I->I) #0:I) #1:I)) (EPS (LAM:I (EQ (APP #2:(I->I) #0:I) #1:I))))) (LAM:I true))) s) F)";
...
fn render_profile_term(
        ...
        .map_err(|error| SignedHolRoundTripError::at("nonsurjective-q-oracle", error))?
    {
        ...
        TermView::Bound { index } => {
            ...
            Ok(format!(
                "#{index}:{}",
                render_type(connection, ty, atoms.ind)?
            ))
        }
        TermView::Application { function, argument } => Ok(format!(
            "(APP {} {})",
            render_profile_term(connection, function, atoms)?,
            render_profile_term(connection, argument, atoms)?
        )),
        TermView::Lambda {
            parameter_type,
            body,
            ...
            "(LAM:{} {})",
            render_type(connection, parameter_type, atoms.ind)?,
            render_profile_term(connection, body, atoms)?
        )),
        TermView::Equality { left, right } => Ok(format!(
            "(EQ {} {})",
            render_profile_term(connection, left, atoms)?,
            render_profile_term(connection, right, atoms)?
        )),
        TermView::Epsilon { predicate } => Ok(format!(
            "(EPS {})",
            render_profile_term(connection, predicate, atoms)?
        )),
        TermView::Free { .. }
        | TermView::Constant { .. }
        | TermView::TypeLambda { .. }
...
fn recover_exact_conjuncts(
    ...
    let TermView::Equality {
        ...
        return Err(SignedHolRoundTripError::invalid(
            "nonsurjective-property-shape",
            "NOT(SURJ successor) is not an equality",
        ));
    };
    if connection
        ...
        || connection
            ...
            .map_err(|error| SignedHolRoundTripError::at("nonsurjective-property-shape", error))?
            != bool_type
        || exact_falsehood != source.falsehood
        || !connection
...
mod tests {
    ...
    fn derives_persists_signs_receives_and_reopens_right_conjunct() {
        ...
        let child = directory
            ...
            .hol_mut()
            .unwrap();
        let TermView::Equality { right, .. } = child.term(artifact.conclusion()).unwrap() else {
            panic!("derived non-surjectivity must be logical negation")
        };
    ...
    fn complete_profile_rejects_unreachable_primitive_false() {
        let kernel = Kernel::ephemeral();
        let artifact = produce_signed_nonsurjective_conjunct(&kernel).unwrap();
        let raw = covalence_neutron::Connection::deserialize(
            &covalence_neutron::Bytes::copy_from_slice(artifact.artifact().image()),
        )
        .unwrap();
