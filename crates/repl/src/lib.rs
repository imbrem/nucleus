...
impl ConnectionId {
    ...
    /// Reconstructs a handle from its displayed number.
    #[must_use]
...
impl Repl {
    ...
    pub fn uri(&self, address: O256) -> String {
        // One form, and the shortest one that works. The mount answers every
        // open with READ_ONLY whatever was asked for, so `mode=ro` and
        // `immutable=1` would add nothing but length to something people have
        // to type.
        format!("file:{}?vfs={}", address.hex(), self.mount.name().as_str())
    }
    ...
    pub fn open_address(&mut self, address: O256) -> Result<ConnectionId, ReplError> {
        ...
        let path = sql::c_string(&uri)?;
        let connection =
            Connection::open_with_flags(&path, OpenFlags::READ_ONLY | OpenFlags::URI, None)?;

        // The URI asked for this mount. This is the check that it got it.
        if connection.database_vfs(c"main")? != self.mount.identity() {
    ...
    pub fn database_vfs(&self, id: ConnectionId, schema: &str) -> Result<VfsIdentity, ReplError> {
        let schema = sql::c_string(schema)?;
...
mod tests {
    ...
    use covalence_lib_sqlite::Statement;
    ...
    fn database_image() -> Vec<u8> {
        ...
        .unwrap();
        connection.serialize(c"main").unwrap().to_vec()
    }
    ...
    fn scalar(
        connection: &Connection,
        statement_sql: &str,
    ) -> Result<i64, covalence_lib_sqlite::Error> {
        let mut statement = Statement::prepare(connection, statement_sql)?;
    ...
    fn the_mount_is_read_only() {
        ...
        let id = repl.open_address(address).unwrap();
        assert!(
            Statement::execute_batch(repl.connection(id).unwrap(), "INSERT INTO value VALUES (7)")
                .is_err()
        );
    }
    ...
    fn several_connections_coexist_and_are_independently_closable() {
        ...
        Statement::execute_batch(
            repl.connection(first).unwrap(),
            "CREATE TABLE only_in_first (n INTEGER)",
        )
        .unwrap();
