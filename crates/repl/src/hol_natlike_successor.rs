...
fn verify_no_primitive_false(
    ...
    if primitive_false_rows != 0 {
        return Err(SignedHolRoundTripError::invalid(
            "natlike-successor-profile-checked",
            "complete successor-closure image contains primitive Boolean false",
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    fn derives_signs_receives_and_reopens_exact_successor_closure() {
        ...
        assert!(
            artifact
                .attestation_text()
                .starts_with("authority=kernel-derived-theorem\nfalsehood=all-bool-identity\n")
        );
        assert!(
            artifact
                .attestation_text()
        ...
        .unwrap();
        verify_no_primitive_false(raw.sqlite()).unwrap();
    }
    ...
    fn derivation_is_independent_of_every_inherited_theorem_handle() {
        ...
        let raw = covalence_neutron::Connection::deserialize(
            &covalence_neutron::Bytes::copy_from_slice(after.image().bytes()),
        )
        .unwrap();
        verify_no_primitive_false(raw.sqlite()).unwrap();
    }
    ...
    fn complete_profile_rejects_unreachable_primitive_false() {
        let kernel = Kernel::ephemeral();
        let artifact = produce_signed_natlike_successor(&kernel).unwrap();
        let raw = covalence_neutron::Connection::deserialize(
            &covalence_neutron::Bytes::copy_from_slice(artifact.artifact().image()),
        )
        .unwrap();
