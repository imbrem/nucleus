...
//! # Kernels
//!
//! A session talks to a kernel. One is always present — the store inside this
//! process — and `(connect …)` adds others reachable over HTTP. The point is
//! that "local" is not a special case: it is the kernel you happen to be
//! running inside, selected the same way as any other.
//!
