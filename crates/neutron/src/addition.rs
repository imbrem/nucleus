...
const ADDITION_INTERPRETATION: &str = "cov.addition/v0";
const ROWID_PARAMETER: &str = "rowid";
const WITHOUT_ROWID_PARAMETER: &str = "without-rowid";
...
impl AdditionLayout {
    const fn parameter(self) -> &'static str {
        match self {
            Self::RowId => ROWID_PARAMETER,
            Self::WithoutRowId => WITHOUT_ROWID_PARAMETER,
        }
    }

    fn from_parameter(value: &str) -> Option<Self> {
        match value {
            ROWID_PARAMETER => Some(Self::RowId),
            WITHOUT_ROWID_PARAMETER => Some(Self::WithoutRowId),
            _ => None,
        }
    }
}
...
impl Connection {
    ...
    pub fn create_addition_table(
        ...
        transaction
            .execute(
                "INSERT INTO cov_catalog (table_name, interpretation, parameters)
                 VALUES (?1, ?2, ?3)",
                (name, ADDITION_INTERPRETATION, layout.parameter()),
            )
            .context(CreateSnafu)?;
    ...
    pub fn validate_addition_tables(&self) -> Result<Vec<AdditionTable>, AdditionError> {
        ...
        let mut statement = self
            .sqlite()
            .prepare(
                "SELECT table_name, interpretation, parameters
                 FROM cov_catalog ORDER BY table_name",
            )
            .context(ScanSnafu)?;
        ...
        entries
            .into_iter()
            .map(|(name, interpretation, parameters)| {
                if interpretation != ADDITION_INTERPRETATION {
                    return Err(AdditionError::UnknownInterpretation {
                        table: name.clone(),
                        interpretation,
                    });
                }
                let layout = AdditionLayout::from_parameter(&parameters).ok_or(
                    AdditionError::UnknownParameters {
                        table: name.clone(),
                        parameters,
                    },
                )?;
...
fn validate_catalog(connection: &sqlite::Connection) -> Result<(), AdditionError> {
    ...
    if columns
        != [
            ...
            (String::from("parameters"), String::from("TEXT"), true, 0),
        ]
    {
...
pub enum AdditionError {
    ...
    /// A catalog entry has unsupported parameters.
    #[snafu(display("table {table:?} has unsupported parameters {parameters:?}"))]
    UnknownParameters {
        /// Physical table name.
        table: String,
        /// Unsupported parameter descriptor.
        parameters: String,
    },
