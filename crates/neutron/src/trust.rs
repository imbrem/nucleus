...
impl Connection {
    ...
    pub fn create_trusted_snapshot_table(&mut self, name: &str) -> Result<(), TrustMetadataError> {
        if !name.starts_with("cov_conn_") || name.starts_with("sqlite_") {
        ...
        register_table(&transaction, id, name, TRUSTED_SNAPSHOTS_INTERPRETATION)
            .map_err(|error| TrustMetadataError::Register { source: error })?;
    ...
    pub fn record_trusted_snapshot(
        ...
        let registered = self
            ...
            .context(QuerySnafu)?;
        if !registered {
            return Err(TrustMetadataError::NotSnapshotTable {
                name: table.to_owned(),
            });
        }
    ...
    /// Tests whether a table records a snapshot as trusted.
    ///
    /// # Errors
    ///
    /// Returns an error when metadata cannot be queried.
