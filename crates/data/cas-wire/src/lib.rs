//! The CAS interface, carried over a byte stream.
//!
//! This is [`covalence_data_cas::Cas`] as a wire protocol, so a client in a
//! different process can read content-addressed objects without sharing an
//! address space with whoever holds them. It is the concrete form of the
//! `covalence:cas/store` WIT interface for a pipe or socket transport.
//!
//! # Handles, not addresses
//!
//! The protocol mirrors the trait rather than flattening it: [`Request::Open`]
//! resolves an address once and returns a handle, and reads name the handle.
//! That is what carries the trait's guarantee across the boundary — the server
//! holds the object open for as long as the client holds the handle, so an
//! address dropped from the store mid-query cannot break a database the client
//! already has open.
//!
//! A flattened read-by-address protocol would have been smaller and would have
//! silently lost that property.
//!
//! # Trust
//!
//! The client is not trusted by the server and does not need to be. Every
//! request names an object the server chose to serve; a client cannot ask for
//! anything else, cannot write, and cannot enumerate. The server bounds
//! response sizes, so a client cannot use a large range to exhaust it.
...
/// Largest range a single [`Request::Read`] may ask for.
///
/// `SQLite` reads a page at a time, so this is generous. It exists because the
/// server must not let one request size its allocation.
...
pub enum Response {
    ...
    ///
    /// Not an error: it says this server did not produce the object, not that
