...
/// # Why an `IndexMap`
///
/// Two reasons, one of which is already load-bearing.
///
/// - **Iteration is deterministic.** [`addresses`](Self::addresses) returns
///   insertion order rather than whatever a hash seed produced, so anything
///   printing or storing a listing gets the same answer twice.
/// - **It leaves room for a store-local integer id.** An index into this map
///   is a compact handle for an object, which is the shape a future optional
///   API would take. What guarantees such an id could carry is *not* settled
///   here, and the choice of removal below is why.
...
impl MemoryCas {
    ...
    pub fn remove(&self, address: O256) -> bool {
        // `shift_remove`, not `swap_remove`: it is O(n) but it keeps the
        // remaining objects in insertion order, which is the only thing that
        // makes an index worth anything. Note that it still *renumbers* every
        // object after the removed one -- so an index is a position, not an
        // identity, and no id built on it can be stable across removal
        // without more than an `IndexMap` underneath.
