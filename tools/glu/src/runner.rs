...
use crate::{BuildTarget, buck, cargo, lean_audit, loc};
...
impl Runner {
    pub(crate) const fn new(root: PathBuf, verbose: u8) -> Self {
        Self {
            ...
            buck_configured: Cell::new(false),
        }
    }
    ...
    pub(crate) fn test(&self, container: bool, cargo: bool) -> Result<()> {
        ...
        self.ensure_buck_configuration()?;
    ...
    pub(crate) fn buck_configure(&self) -> Result<()> {
        ...
        self.buck_configured.set(true);
        eprintln!("configured Buck tool paths");
        Ok(())
    }

    /// Ensure Buck sees absolute native tool paths before it creates compiler
    /// shims for Cargo build scripts.
    ///
    /// Buck's bundled shim helper executes the configured compiler directly,
    /// without searching `PATH`. The development shell writes these paths on
    /// entry, but a newly-created Git worktree can reuse the shell without
    /// running its hook. Generate or refresh glu's local configuration here so
    /// every glu-driven Buck command also works in that case. A user-authored
    /// local configuration remains authoritative.
    fn ensure_buck_configuration(&self) -> Result<()> {
        if self.buck_configured.get() {
            return Ok(());
        }
        ...
        if path.exists() {
            ...
            if !is_generated_buck_configuration(&current) {
                self.buck_configured.set(true);
                return Ok(());
            }
        }
    ...
    fn write_buck_configuration(&self) -> Result<()> {
        ...
        if path.exists() {
            ...
            let generated = is_generated_buck_configuration(&current);
            if !generated {
                bail!(
...
fn is_generated_buck_configuration(configuration: &str) -> bool {
    matches!(
        configuration.lines().next(),
...
fn command_path(name: &str) -> Result<PathBuf> {
    ...
    if path.is_absolute() {
        ...
        Ok(env::current_dir()
            .wrap_err("could not resolve the current directory")?
            .join(path))
    }
}
...
mod tests {
    ...
    #[test]
    fn recognizes_only_glu_generated_buck_configuration() {
        assert!(is_generated_buck_configuration(&format!(
            "{BUCK_CONFIG_HEADER}\n[nucleus]\n"
        )));
        assert!(is_generated_buck_configuration(
            "# Generated by `glu buck configure`.\n[nucleus]\n"
        ));
        assert!(!is_generated_buck_configuration(
            "[nucleus]\ncc = /custom/clang\n"
        ));
